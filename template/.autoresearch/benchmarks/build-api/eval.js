...
/**
 * eval.js — LOCKED benchmark evaluator for build-api.
 *
...
function evaluate(dir) {
  const details = {
    ...
    testsPassed: 0,
    testsRan: false,
    filesExpected: ['lib/routes/users.js'],
    filesFound: [],
    goalsUpdated: false,
    hasAttempt: false,
    pipelineState: null,
  ...
  try {
    ...
    const testResult = execSync('node --test tests/users.test.js', {
      cwd: dir,
      encoding: 'utf-8',
      timeout: 30000,
  ...
  if (!details.testsRan || details.testsTotal === 0) {
    try {
      const fallback = execSync('node --test tests/users.test.js 2>&1 || true', {
        cwd: dir,
        encoding: 'utf-8',
        timeout: 30000,
        shell: true,
