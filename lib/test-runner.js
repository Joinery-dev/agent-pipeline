...
function getChangedFiles() {
  try {
    ...
    console.error(`[test-runner] getChangedFiles: git command failed: ${err.message}${err.stderr ? ' | stderr: ' + err.stderr.toString().slice(0, 200) : ''}`);
    return [];
  }
}
...
function detectLinter() {
  ...
  if (eslintConfigs.some(f => existsSync(f))) return 'eslint';

  // Biome
  if (existsSync('biome.json') || existsSync('biome.jsonc')) return 'biome';

  // Prettier (check-only, not a linter but catches formatting)
  const prettierConfigs = ['.prettierrc', '.prettierrc.js', '.prettierrc.json', '.prettierrc.yml', '.prettierrc.yaml'];
  if (prettierConfigs.some(f => existsSync(f))) return 'prettier';
  try {
    const pkg = JSON.parse(readFileSync('package.json', 'utf-8'));
    if (pkg.prettier || pkg.devDependencies?.prettier || pkg.dependencies?.prettier) return 'prettier';
  } catch (err) {
    console.error(`[test-runner] detectLinter: failed to read/parse package.json: ${err.message}`);
  }
...
function runLintCheck() {
  ...
  if (!linter) return { skipped: true, reason: 'No linter config detected' };
  ...
  try {
    execSync(cmd, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 60000 });
    return { passed: true, linter };
  } catch (err) {
    const output = ((err.stdout || '') + '\n' + (err.stderr || '')).trim();
    return { passed: false, linter, error: output.slice(0, 500) };
  }
}
...
// Lint check (all tiers) — detect linter and run in check mode
const lintResult = runLintCheck();
if (lintResult && !lintResult.passed && !lintResult.skipped) {
  // Don't fail the run, but include lint info in output
}
...
// Attach lint result
if (lintResult && !lintResult.skipped) {
  result.lint = lintResult;
  if (!lintResult.passed) {
    result.failed += 1;
    result.failures.push({ name: 'Lint check failed', details: lintResult.error });
  }
}
...
function runBuildCheck() {
  ...
  try {
    ...
    console.error(`[test-runner] runBuildCheck: failed to read/parse package.json: ${err.message}`);
  }
...
function runE2ETests() {
  ...
  try {
    ...
    try {
      ...
      console.error(`[test-runner] runE2ETests: failed to parse Playwright JSON output: ${err.message}`);
