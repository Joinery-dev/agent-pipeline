...
function loadShipConfig() {
  ...
  if (existsSync(configPath)) {
    ...
    } catch (err) { console.error(`[SHIP] ship-config.json parse failed: ${err.message}`); return defaults; }
  }
  return defaults;
}
...
const MAX_REPAIR_ATTEMPTS = CONFIG.maxRepairAttempts;
const REPAIR_SCRIPT_TIMEOUT_MS = CONFIG.repairScriptTimeoutMs;
...
function runLifecycleHooks(event, context = {}) {
  ...
  for (const hook of hooks) {
    try {
      ...
      // Hook failure must never break the pipeline, but log for diagnostics
      logVerbose('ship', `Hook ${event}/${script} failed: ${err.message}`);
    }
  }
}

// ── Human Notifications (ntfy.sh) ─────────────────────────────────────────

const HUMAN_REQUIRED_PATH = resolve('.ship/human-required.md');

function getNotificationContext() {
  const projectDir = process.cwd().split('/').pop();
  const goals = readGoals();
  const projectName = goals?.name || projectDir;

  // Find the active phase
  const activePhase = goals ? getAllPhases(goals).find(p =>
    p.status !== 'completed'
  ) : null;

  const phaseLine = activePhase
    ? `Phase: "${activePhase.title}" (${activePhase.pipeline?.state || activePhase.status})`
    : 'No active phase';

  return { projectName, phaseLine, projectDir };
}

function sendNotification(title, message, priority) {
  const notifyConfig = CONFIG.notify;
  if (!notifyConfig?.enabled || !notifyConfig?.ntfyTopic) return;

  const topic = notifyConfig.ntfyTopic;
  const prio = priority || notifyConfig.priority || 'high';
  const tags = (notifyConfig.tags || ['robot']).join(',');

  try {
    execSync(
      `curl -s -o /dev/null -H "Title: ${title.replace(/"/g, '\\"')}" -H "Priority: ${prio}" -H "Tags: ${tags}" -d "${message.replace(/"/g, '\\"')}" https://ntfy.sh/${topic}`,
      { stdio: 'pipe', timeout: 10000 }
    );
    log('ship', `Notification sent: ${title}`);
  } catch (err) {
...
/**
 * Check for human-required flags written by agents.
 * Agents write to .ship/human-required.md when they encounter issues
 * that no agent can resolve (missing credentials, business decisions, etc.)
 *
 * Returns true if human attention is needed (pipeline should pause).
 */
function checkHumanRequired() {
  if (!existsSync(HUMAN_REQUIRED_PATH)) return false;

  const content = readFileSync(HUMAN_REQUIRED_PATH, 'utf-8').trim();
  if (!content) return false;

  log('ship', `HUMAN REQUIRED — agents flagged issues needing human attention`);
  const ctx = getNotificationContext();
  sendNotification(
    `${ctx.projectName}: Pipeline needs you`,
    `${ctx.phaseLine}\n\n${content.slice(0, 400)}\n\nTo resume: fix the issue, then:\nrm .ship/human-required.md && node lib/ship.js --resume`,
    'urgent'
  );
...
/**
 * Also scan QA memory for HUMAN-REQUIRED findings that the QA agent
 * may have logged without writing to the flag file.
 */
function checkQAForHumanRequired() {
  const statusPath = resolve('.qa/memory/status.json');
  if (!existsSync(statusPath)) return false;

  try {
    const status = JSON.parse(readFileSync(statusPath, 'utf-8'));

    // Check forest warnings for HUMAN-REQUIRED category
    const humanWarnings = (status.forestWarnings || []).filter(w =>
      w.category === 'human-required' || /human.required|missing.*key|missing.*credential|missing.*config|no.*api.*key/i.test(w.description || '')
    );

    if (humanWarnings.length === 0) return false;

    const message = humanWarnings
      .map(w => `[${w.risk}] ${w.description}`)
      .join('\n');

    // Write flag file so it persists across restarts
    mkdirSync(resolve('.ship'), { recursive: true });
    writeFileSync(HUMAN_REQUIRED_PATH, `# Human Required\n\n${message}\n\nFlagged: ${new Date().toISOString()}\n`);

    log('ship', `HUMAN REQUIRED — QA found ${humanWarnings.length} issue(s) needing human attention`);
    const ctx = getNotificationContext();
    sendNotification(
      `${ctx.projectName}: Pipeline needs you`,
      `${ctx.phaseLine}\n\n${message.slice(0, 400)}\n\nTo resume: fix the issue, then:\nrm .ship/human-required.md && node lib/ship.js --resume`,
...
let logStream = null;
let activeProgressSync = null; // current progress sync timer, if any
const STATUS_PATH = resolve('.ship/status.json');

function writeStatus(state, extra = {}) {
  try {
    const status = { state, timestamp: new Date().toISOString(), pid: process.pid, ...extra };
    mkdirSync(resolve('.ship'), { recursive: true });
...
function clearPhaseForReplan(phase) {
  try {
    ...
    log('exec', `Phase clear failed: ${stderr}`);
  }
}
...
function getFailedTasks(phase) {
  return phase.tasks
    .filter(t => {
      ...
      // Also catch tasks where the latest QA attempt failed but the task
      // wasn't set to blocked (common — reconcileQAStatuses only sets
      // successes to completed, doesn't touch failures)
      if (t.status === 'completed') return false;
      const attempts = t.attempts || [];
      const lastQA = [...attempts].reverse().find(a =>
        a.type === 'qa' || a.type === 'qa-recheck'
      );
      return lastQA && (lastQA.outcome === 'failure' || lastQA.outcome === 'partial');
    })
    ...
    if (existsSync(fullDir)) {
      try {
        ...
        for (const f of files) {
          try { totalChars += readFileSync(resolve(fullDir, f), 'utf-8').length; } catch (err) { logVerbose('ship', `Memory file read failed ${f}: ${err.message}`); }
        }
      } catch (err) { logVerbose('ship', `Memory dir scan failed ${dir}: ${err.message}`); }
    }
  }
...
function runDistiller(agentType, taskId, { phaseId, outputPath, files } = {}) {
  ...
  try {
    ...
    const stderr = err.stderr?.toString().trim() || err.message;
    logVerbose('ship', `Distiller skipped: ${stderr}`);
  }
}
...
function runPlanValidator(phaseId) {
  ...
  try {
    ...
    const stderr = err.stderr?.toString().trim() || err.message;
    logVerbose('ship', `Plan validator failed: ${stderr}`);
...
function runTestGate(tier = 1) {
  ...
  try {
    ...
    try {
      ...
      logVerbose('ship', `Test gate output parse failed: ${parseErr.message}`);
      return { passed: false, result: { failed: 1, message: stderr } };
    }
  }
}
...
function runMemoryHygiene() {
  ...
  try {
    ...
    const stderr = err.stderr?.toString().trim() || err.message;
    logVerbose('ship', `Memory hygiene failed: ${stderr}`);
  }
}
...
async function runResearch(topic, { force = false, failureContext = null } = {}) {
  ...
  if (!force) {
    ...
    } catch (err) { logVerbose('ship', `Research brief scan failed: ${err.message}`); }
  }
...
// ── Forest Warning Resolution ────────────────────────────────────────────

const PIPELINE_WARNING_PATTERNS = [
  /stale.*attempt/i,
  /in-progress.*empty/i,
  /pipeline.*hygiene/i,
  /\.goals\.json.*schema/i,
  /attempt.*not.*closed/i,
  /orphan.*task/i,
  /missing.*attempt/i,
  /round.*tracking/i,
];

function isPipelineWarning(warning) {
  // Trust explicit category if QA agent set it
  if (warning.category === 'pipeline') return true;
  if (warning.category === 'code') return false;
  // Fallback: pattern match for unclassified warnings
  const text = `${warning.description || ''} ${warning.context || ''} ${warning.suggestion || ''}`;
  return PIPELINE_WARNING_PATTERNS.some(p => p.test(text));
}

/**
 * Fix pipeline hygiene warnings deterministically (no LLM needed).
 * Handles stale attempts, missing notes, etc.
 */
function fixPipelineWarnings(warnings, phaseId) {
  if (warnings.length === 0) return;
  log('ship', `${warnings.length} pipeline warning(s) — fixing deterministically`);
  ...
  for (const w of warnings) {
    ...
    // Fix stale in-progress attempts
    if (/stale.*attempt|attempt.*in-progress|in-progress.*empty/i.test(desc)) {
      for (const task of (phase.tasks || [])) {
        for (const attempt of (task.attempts || [])) {
          if (attempt.outcome === 'in-progress') {
            attempt.outcome = 'partial';
            attempt.notes = (attempt.notes || '') + '\n[ship.js] Closed stale in-progress attempt (QA forest warning)';
            fixed++;
          }
        }
      }
  ...
  if (fixed > 0) {
    ...
    log('ship', `Fixed ${fixed} pipeline hygiene issue(s)`);
  }
}

/**
 * Read forest warnings from QA memory. Route code warnings to resolver,
 * fix pipeline warnings deterministically.
 */
async function resolveForestWarnings(phaseId) {
  const statusPath = resolve('.qa/memory/status.json');
  if (!existsSync(statusPath)) return false;

  let status;
  try { status = JSON.parse(readFileSync(statusPath, 'utf-8')); } catch (err) { logVerbose('ship', `QA status.json parse failed: ${err.message}`); return false; }

  // Filter out LOW and human-required (handled separately by checkQAForHumanRequired)
  const warnings = (status.forestWarnings || []).filter(w =>
    w.risk !== 'LOW' && w.category !== 'human-required'
  );
  if (warnings.length === 0) return false;

  // Split into code warnings (resolver) and pipeline warnings (deterministic)
  const codeWarnings = warnings.filter(w => !isPipelineWarning(w));
  const pipelineWarnings = warnings.filter(w => isPipelineWarning(w));

  // Fix pipeline warnings deterministically
  fixPipelineWarnings(pipelineWarnings, phaseId);

  // Route code warnings to resolver
  if (codeWarnings.length > 0) {
    log('ship', `${codeWarnings.length} code warning(s) → resolver, ${pipelineWarnings.length} pipeline warning(s) → fixed`);

    const warningList = codeWarnings
      .map(w => `- [${w.risk}] ${w.description}${w.suggestion ? ` — Suggestion: ${w.suggestion}` : ''}`)
      .join('\n');

    runDistiller('resolve', null);
    await runAgent(
      `/resolve QA forest warnings need fixing for phase ${phaseId}. These are code quality findings that should be resolved before the phase is complete:\n\n${warningList}\n\nFix each warning. Do NOT change functionality — only address the specific issues listed.`,
      `Resolving ${codeWarnings.length} code warning(s)`
    );
  } else {
    log('ship', `${pipelineWarnings.length} pipeline warning(s) fixed — no code warnings`);
  }

  // Clear resolved warnings from QA memory
  try {
    const fresh = JSON.parse(readFileSync(statusPath, 'utf-8'));
    fresh.forestWarnings = [];
    writeFileSync(statusPath, JSON.stringify(fresh, null, 2));
  } catch (err) { log('error', `Failed to clear forest warnings: ${err.message}`); }
...
function reconcileQAStatuses(phaseId) {
  try {
    ...
    log('error', `QA reconciliation error: ${err.message}`);
  }
}
...
function logDecision(message) {
  ...
  try {
    ...
    logVerbose('ship', `Decision log write failed: ${err.message}`);
  }
}
...
function runVisualCheck(mode, phaseId) {
  ...
  try {
    ...
    if (output) {
      ...
      } catch (parseErr) { logVerbose('ship', `Visual check output parse failed: ${parseErr.message}`); }
    }
...
function runPlaywrightGate(phaseId) {
  const gatePath = resolve('lib/playwright-gate.js');
  if (!existsSync(gatePath)) return { skipped: true, reason: 'playwright-gate.js not found' };

  const args = [];
  if (phaseId) args.push('--dir', 'tests/qa');

  try {
    const result = execFileSync('node', [gatePath, ...args], {
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'pipe'],
      timeout: 120000,
    });
    ...
    if (parsed.skipped) {
      logVerbose('ship', `Playwright gate skipped: ${parsed.reason || 'no tests'}`);
      return parsed;
    }

    if (parsed.passed) {
      log('ship', `Interactive tests passed: ${parsed.pass}/${parsed.total}`);
    } else {
      log('error', `Interactive tests failed: ${parsed.fail} failures`);
      for (const f of (parsed.failingFiles || []).slice(0, 5)) {
        log('error', `  ${f}`);
      }
    }
    ...
    logVerbose('ship', `Playwright gate error: ${err.message}`);
    return { skipped: true, reason: err.message };
  }
}
...
function reconcileTaskStatuses(phaseId) {
  try {
    ...
    try {
      ...
      logVerbose('ship', `git diff failed during reconciliation: ${err.message}`);
      changedFiles = [];
    }
    ...
    } catch (err) { logVerbose('ship', `git ls-files failed during reconciliation: ${err.message}`); }
...
function cleanupCheckpoints(maxIteration) {
  ...
  try {
    ...
    logVerbose('ship', `Checkpoint cleanup: ${err.message}`);
  }
}

function listCheckpointsCommand() {
  try {
    const raw = execSync('git tag -l "ship/iter-*" --sort=version:refname', { encoding: 'utf-8' }).trim();
    if (!raw) {
      console.log('No checkpoints found. Checkpoints are created during pipeline runs and cleaned up on success.');
      return;
    }
    const tags = raw.split('\n');
    console.log(`\n  Checkpoints (${tags.length}):\n`);
    for (const tag of tags) {
      const iter = tag.replace('ship/iter-', '');
      let date = '';
      try {
        date = execSync(`git log -1 --format="%ci" ${tag}`, { encoding: 'utf-8' }).trim();
      } catch (err) { logVerbose('ship', `Checkpoint date lookup failed for ${tag}: ${err.message}`); }
      let subject = '';
      try {
        subject = execSync(`git log -1 --format="%s" ${tag}`, { encoding: 'utf-8' }).trim();
        if (subject.length > 60) subject = subject.slice(0, 57) + '...';
      } catch (err) { logVerbose('ship', `Checkpoint subject lookup failed for ${tag}: ${err.message}`); }
      console.log(`    iter-${iter.padEnd(4)} ${date ? date.slice(0, 19) : '(no date)'}  ${subject}`);
    }
    console.log(`\n  Restore with: node lib/ship.js --restore <N>\n`);
  } catch (err) {
...
function restoreCheckpointCommand(n) {
  const tag = `ship/iter-${n}`;
  try {
    execSync(`git rev-parse ${tag}`, { stdio: 'pipe' });
  } catch {
    console.error(`Checkpoint ${tag} does not exist. Run --checkpoints to see available checkpoints.`);
    process.exit(1);
  }
  const files = ['.goals.json', '.pm/memory/', '.qa/memory/'];
  const restored = [];
  for (const f of files) {
    try {
      execSync(`git checkout ${tag} -- ${f}`, { stdio: 'pipe' });
      restored.push(f);
    } catch (err) {
      logVerbose('ship', `Restore skipped ${f} from ${tag}: ${err.message}`);
    }
  }
  if (restored.length === 0) {
    console.error(`No state files found at ${tag}.`);
    process.exit(1);
  }
  console.log(`Restored from ${tag}: ${restored.join(', ')}`);
  try {
    const subject = execSync(`git log -1 --format="%s" ${tag}`, { encoding: 'utf-8' }).trim();
    console.log(`  Checkpoint commit: ${subject}`);
  } catch (err) { logVerbose('ship', `Checkpoint subject lookup failed: ${err.message}`); }
}
...
async function runCanary() {
  ...
  try {
    ...
    try { unlinkSync(canaryPath); } catch (err) { logVerbose('ship', `Canary cleanup failed: ${err.message}`); }
  }
}
...
function detectAndResetStaleTasks(staleTimeoutMs) {
  ...
  if (existsSync(PIPELINE_CLI)) {
    try {
      ...
      const stderr = err.stderr?.toString().trim() || err.message;
      logVerbose('ship', `Stale task detection via CLI failed, falling back: ${stderr}`);
    }
  }
...
function getProgressSnapshot() {
  ...
  } catch (err) { logVerbose('ship', `Progress snapshot git diff failed: ${err.message}`); }
  ...
  } catch (err) { logVerbose('ship', `Progress snapshot git ls-files failed: ${err.message}`); }

  try {
    goalsJson = readFileSync(GOALS_PATH, 'utf-8');
  } catch (err) { logVerbose('ship', `Progress snapshot goals read failed: ${err.message}`); }
...
async function runAgent(command, description, { cwd: agentCwd } = {}) {
  ...
  // ── Post-run: cost tracking, goals diff, result interpretation ──
  ...
  try {
    const costEntry = recordCost({
      agent: agentType,
      command,
      phaseId: null, // filled in by caller if available
      taskTitle: description,
      usage: result.usage,
      durationMs: result.durationMs,
    });
    if (costEntry) {
      logVerbose('ship', `Cost: ${costEntry.totalTokens} tokens ($${costEntry.estimatedCost.toFixed(4)})`);
    }
  } catch (err) {
    logVerbose('ship', `Cost tracking failed: ${err.message}`);
  }
  ...
  } catch (err) { logVerbose('ship', `Exec history save failed: ${err.message}`); }
...
function findParallelDesignPhases(goals) {
  const phases = getAllPhases(goals).filter(p => {
    const state = getPipelineState(p);
    return state === 'awaiting-design';
  });
  ...
  if (independent.length < 2) return null;
  return independent.slice(0, MAX_PARALLEL_QA); // reuse same concurrency limit
}
...
function createWorktree(phase, prefix = 'build') {
  ...
  try { execSync(`git worktree remove "${worktreePath}" --force 2>/dev/null`, { stdio: 'pipe' }); } catch (err) { logVerbose('ship', `Worktree pre-cleanup: ${err.message}`); }
...
function commitWorktreeChanges(worktreePath, phaseTitle) {
  ...
  try {
    ...
    } catch (err) { logVerbose('ship', `Worktree recovery git scan failed: ${err.message}`); }
    ...
    for (const file of mainChanged) {
      try {
        ...
        copied++;
      } catch (err) { logVerbose('ship', `Worktree file copy failed ${file}: ${err.message}`); }
    }
    ...
    for (const file of mainChanged) {
      try {
        ...
        logVerbose('ship', `Worktree git checkout reset failed for ${file}: ${err.message}`);
        try { unlinkSync(resolve(file)); } catch (unlinkErr) { logVerbose('ship', `Worktree unlink failed for ${file}: ${unlinkErr.message}`); }
      }
    }
...
function mergeWorktreeBranch(branchName, phaseTitle) {
  ...
  try {
    ...
    log('ship', `Git merge conflict for "${branchName}" — falling back to file-level merge`);
    try { execSync('git merge --abort', { stdio: 'pipe' }); } catch (abortErr) { logVerbose('ship', `Merge abort failed: ${abortErr.message}`); }
  }

  // Step 5: File-level merge fallback
  // When multiple builders modify the same file (e.g., app/page.js), git can't
  // auto-merge. Extract each file from the branch individually. Files that were
  // also modified by a previous builder are marked as conflicts for reconciliation.
  try {
    const mergeBase = execSync(`git merge-base HEAD "${branchName}"`, { encoding: 'utf-8', stdio: 'pipe' }).trim();
    const branchFiles = execSync(`git diff --name-only "${mergeBase}".."${branchName}"`, { encoding: 'utf-8', stdio: 'pipe' })
      .trim().split('\n').filter(Boolean);
    ...
    for (const file of branchFiles) {
      try {
        // Get branch's version of this file
        const content = execSync(`git show "${branchName}":"${file}"`, { encoding: 'buffer', stdio: ['pipe', 'pipe', 'pipe'] });

        // Check if this file was also modified in main since the merge base
        let isConflict = false;
        try {
          const baseContent = execSync(`git show "${mergeBase}":"${file}"`, { encoding: 'buffer', stdio: ['pipe', 'pipe', 'pipe'] });
          const mainContent = readFileSync(resolve(file));
          isConflict = !mainContent.equals(baseContent);
        } catch (baseErr) {
          // File is new (doesn't exist in base) — check if it also exists in main
          logVerbose('ship', `Merge base check for ${file}: ${baseErr.message}`);
          isConflict = existsSync(resolve(file));
        }

        // Write the branch's version (may overwrite a previous builder's version)
        ...
        if (isConflict) {
          conflicts.push(file);
          log('ship', `  Conflict: ${file} (modified by previous builder AND "${phaseTitle}")`);
        }
      } catch (fileErr) {
        logVerbose('ship', `  Skipped ${file}: ${fileErr.message}`);
      }
    }

    if (applied > 0) {
      execSync('git add -A', { stdio: 'pipe' });
      execSync(`git commit -m "merge parallel build: ${phaseTitle} (file-level, ${conflicts.length} conflicts)"`, { stdio: 'pipe' });
      log('ship', `Applied ${applied} files from "${branchName}" (${conflicts.length} need reconciliation)`);
    }

    return { success: applied > 0, conflicts };
  } catch (err) {
    log('error', `File-level merge failed for "${branchName}": ${err.message}`);
    return { success: false, conflicts: [] };
  }
}

function cleanupWorktree(worktreePath, branchName) {
  try { execSync(`git worktree remove "${worktreePath}" --force`, { stdio: 'pipe' }); } catch (err) { logVerbose('ship', `Worktree remove failed: ${err.message}`); }
  try { execSync(`git branch -D "${branchName}"`, { stdio: 'pipe' }); } catch (err) { logVerbose('ship', `Branch cleanup failed: ${err.message}`); }
}
...
function reconcileParallelGoals(worktrees) {
  ...
  for (const { path: wtPath, phaseId } of worktrees) {
    try {
      ...
      if (!existsSync(wtGoalsPath)) {
        logVerbose('ship', `Goals reconciliation: ${wtPath}/.goals.json not found — skipping`);
        continue;
      }
      ...
      log('ship', `Goals reconciliation failed for ${wtPath}: ${err.message}`);
    }
  }

  writeFileSync(GOALS_PATH, JSON.stringify(mainGoals, null, 2));
  if (totalUpdated > 0) {
    logVerbose('ship', `Reconciled .goals.json: ${totalUpdated} task updates from ${worktrees.length} worktrees`);
  }
}
...
function startWorktreeProgressSync(worktrees, intervalMs = 30000) {
  const sync = () => {
    ...
    } catch (err) { logVerbose('ship', `Worktree progress sync failed: ${err.message}`); }
  };
  const timer = setInterval(sync, intervalMs);
  const handle = { stop: () => { clearInterval(timer); activeProgressSync = null; } };
  activeProgressSync = handle;
  return handle;
}

function pauseProgressSync() {
  if (activeProgressSync) { activeProgressSync.stop(); }
}
...
async function runParallelBuilds(phases) {
  ...
  for (const phase of phases) {
    try {
      ...
      // Attempt repair for worktree creation failures (stale locks, existing branches)
      const repairResult = await attemptRepair(err, {
      ...
      if (repairResult.repaired) {
        try {
          ...
          log('error', `Worktree creation still failed after repair for "${phase.title}": ${retryErr.message}`);
        }
      }
    }
  }
  ...
  const allConflicts = []; // { file, phases[] } — files modified by multiple builders
  ...
  const pipelines = worktrees.map(async (wt) => {
    ...
    await withMergeLock(async () => {
      ...
      // Always attempt merge — mergeWorktreeBranch tries git merge first,
      // falls back to file-level application if conflicts
      let mergeResult = mergeWorktreeBranch(wt.branch, wt.phase.title);
      merged = mergeResult?.success || mergeResult === true;

      // If merge failed, attempt repair and retry
      if (!merged) {
        const repairResult = await attemptRepair(
          new Error(`Merge failed for branch ${wt.branch}`),
          { operation: 'merge', phaseId: wt.phaseId, phaseTitle: wt.phase.title }
        );
        if (repairResult.repaired) {
          mergeResult = mergeWorktreeBranch(wt.branch, wt.phase.title);
          merged = mergeResult?.success || mergeResult === true;
        }
      }

      if (mergeResult?.conflicts?.length > 0) {
        for (const file of mergeResult.conflicts) {
          const existing = allConflicts.find(c => c.file === file);
          if (existing) {
            existing.phases.push(wt.phase.title);
          } else {
            allConflicts.push({ file, phases: [wt.phase.title] });
          }
        }
      }
    ...
    try {
      ...
      if (existsSync(briefingSrc)) {
        ...
        try { unlinkSync(briefingSrc); } catch (err) { logVerbose('ship', `QA briefing cleanup failed: ${err.message}`); }
      }
      ...
      await withMergeLock(async () => {
        ...
        } catch (err) { logVerbose('ship', `QA round tracking failed: ${err.message}`); }

        reconcileQAStatuses(wt.phaseId);
        runLifecycleHooks('post-qa', { phaseId: wt.phaseId });

        // Check if QA flagged anything needing human attention
        if (checkHumanRequired() || checkQAForHumanRequired()) {
          log('ship', 'Pipeline paused — waiting for human. Remove .ship/human-required.md to resume.');
          return; // exit this pipeline's merge lock — main loop will stop
        }
        ...
        if (visualResult.regressions?.length > 0) {
          ...
          if (existsSync(PIPELINE_CLI)) {
            ...
            } catch (err) { log('error', `Failed to set pipeline qa-failed for ${wt.phaseId}: ${err.message}`); }
          }
          return;
        }

        // Interactive test gate — run QA-generated browser test specs
        const interactiveResult = runPlaywrightGate(wt.phaseId);
        if (interactiveResult.passed === false) {
          log('error', `Interactive tests failed on "${wt.phase.title}" — blocking completion`);
          if (existsSync(PIPELINE_CLI)) {
            try {
              execFileSync('node', [PIPELINE_CLI, 'set-pipeline', wt.phaseId, 'qa-failed', '--agent', 'qa'], {
                encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'],
        ...
        // Resolve forest warnings before moving on
        await resolveForestWarnings(wt.phaseId);
        ...
        if (allTasksDone) {
          ...
          if (needsDesign && hasVisualFiles) {
            if (existsSync(PIPELINE_CLI)) {
              ...
              } catch (err) { log('error', `Failed to set pipeline awaiting-design for ${wt.phaseId}: ${err.message}`); }
            }
            ...
            if (existsSync(PIPELINE_CLI)) {
              ...
              } catch (err) { log('error', `Failed to set pipeline complete for ${wt.phaseId}: ${err.message}`); }
            }
  ...
  // ── Reconcile conflicting files from parallel builds ──
  // When multiple builders modify the same file (e.g., app/page.js), the last
  // one merged wins. Spawn a resolver to combine all builders' sections.
  if (allConflicts.length > 0) {
    const conflictList = allConflicts
      .map(c => `- ${c.file} (modified by: ${c.phases.join(', ')})`)
      .join('\n');
    log('ship', `Reconciling ${allConflicts.length} file conflicts from parallel builds:\n${conflictList}`);

    try {
      await runAgent(
        `/resolve Multiple parallel builders independently modified the same files. The current version only has the LAST builder's changes — earlier builders' sections are missing.\n\nConflicting files:\n${conflictList}\n\nUse git log to see what each builder wrote. Combine ALL builders' sections into each conflicting file. Every builder's contribution must be preserved.`,
        'Reconciling parallel build file conflicts'
      );

      // Commit the reconciled result
      try {
        const status = execSync('git status --porcelain', { encoding: 'utf-8', stdio: 'pipe' }).trim();
        if (status) {
          execSync('git add -A', { stdio: 'pipe' });
          execSync('git commit -m "ship.js: reconciled parallel build conflicts"', { stdio: 'pipe' });
          log('ship', 'Parallel build conflicts reconciled and committed');
        }
      } catch (err) { log('error', `Reconciliation commit failed: ${err.message}`); }
    } catch (err) {
      log('error', `Reconciliation agent failed: ${err.message}`);
    }
  }
...
async function runParallelQA(phases) {
  ...
  for (const phase of phases) {
    try {
      ...
      if (existsSync(briefingSrc)) {
        copyFileSync(briefingSrc, resolve(wt.path, '.ship/briefing.md'));
        try { unlinkSync(briefingSrc); } catch (err) { logVerbose('ship', `QA briefing cleanup failed: ${err.message}`); } // clean up temp briefing
      }
  ...
  for (const { wt, result } of results) {
    ...
    if (checkHumanRequired() || checkQAForHumanRequired()) {
      log('ship', 'Pipeline paused — waiting for human. Remove .ship/human-required.md to resume.');
      break; // exit the for loop — main loop will detect shouldStop
    }
...
async function runParallelDesign(phases) {
  log('ship', `Parallel design review: ${phases.length} independent phases`);
  logDecision(`Running ${phases.length} design reviews in parallel: ${phases.map(p => p.title).join(', ')}`);

  // Phase 1: Prepare briefings and worktrees (sequential)
  const worktrees = [];
  for (const phase of phases) {
    try {
      const briefingPath = `.ship/briefing-design-${phase.id}.md`;
      runDistiller('design', phase.id, { outputPath: briefingPath });

      const wt = createWorktree(phase, 'design');
      wt.phaseId = phase.id;
      wt.phase = phase;
      ...
      if (existsSync(briefingSrc)) {
        copyFileSync(briefingSrc, resolve(wt.path, '.ship/briefing.md'));
        try { unlinkSync(briefingSrc); } catch (err) { logVerbose('ship', `Design briefing cleanup failed: ${err.message}`); }
      }

      // Track design rounds per-phase (write to worktree's copy)
      const wtStatusPath = resolve(wt.path, '.design/memory/status.json');
      let designStatus = { round: 0 };
      try {
        const mainStatusPath = resolve('.design/memory/status.json');
        if (existsSync(mainStatusPath)) designStatus = JSON.parse(readFileSync(mainStatusPath, 'utf-8'));
      } catch (err) { logVerbose('ship', `Design status read failed: ${err.message}`); }
      designStatus.round = (designStatus.round || 0) + 1;
      designStatus.phase = phase.title;
      designStatus.lastRun = new Date().toISOString();
      try {
        mkdirSync(resolve(wt.path, '.design/memory'), { recursive: true });
        writeFileSync(wtStatusPath, JSON.stringify(designStatus, null, 2));
      } catch (err) { logVerbose('ship', `Design status write to worktree failed: ${err.message}`); }
      wt.designRound = designStatus.round;

      worktrees.push(wt);
    } catch (err) {
      log('error', `Failed to prepare design worktree for "${phase.title}": ${err.message}`);
    }
  }

  if (worktrees.length === 0) {
    log('error', 'No design worktrees created — falling back to sequential');
    return { success: false, madeProgress: false };
  }

  // Phase 2: Dispatch design review agents in parallel
  const progressSync = startWorktreeProgressSync(worktrees);

  const promises = worktrees.map(async (wt) => {
    const result = await runAgent(
      `/design-review ${wt.phase.title}`,
      `[Parallel Design] Reviewing: ${wt.phase.title}`,
      { cwd: wt.path }
    );
    return { wt, result };
  });
  ...
  // Phase 3: Reconcile (sequential)
  reconcileParallelGoals(worktrees);

  let anyProgress = false;

  // Update design round tracking in main .goals.json
  try {
    const dGoals = readGoals();
    for (const { wt } of results) {
      const dPhase = getAllPhases(dGoals).find(p => p.id === wt.phaseId);
      if (dPhase) {
  ...
  } catch (err) { logVerbose('ship', `Design round tracking update failed: ${err.message}`); }

  for (const { wt, result } of results) {
    if (result?.success || result?.madeProgress) anyProgress = true;

    const hasShipBlockers = result?.output?.includes('SHIP-BLOCKER') || false;
    const designRounds = wt.phase.pipeline?.designRoundsCumulative || wt.designRound || 1;

    if (hasShipBlockers) {
      if (designRounds > MAX_DESIGN_ROUNDS) {
        log('exec', `Design review failed ${designRounds} times on "${wt.phase.title}" — will escalate`);
        // Escalation handled by main loop on next iteration when it sees design-failed state
      }
      log('ship', `Design SHIP-BLOCKERs on "${wt.phase.title}" (round ${designRounds}/${MAX_DESIGN_ROUNDS})`);
      if (existsSync(PIPELINE_CLI)) {
        try {
          execFileSync('node', [PIPELINE_CLI, 'set-pipeline', wt.phaseId, 'design-failed', '--agent', 'design'], {
            encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'],
          });
        } catch (err) { log('error', `Failed to set pipeline design-failed for ${wt.phaseId}: ${err.message}`); }
      }
    } else {
      // Design passed — mark phase complete
      if (existsSync(PIPELINE_CLI)) {
        try {
          execFileSync('node', [PIPELINE_CLI, 'set-pipeline', wt.phaseId, 'complete', '--agent', 'design'], {
            encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'],
          });
        } catch (err) { log('error', `Failed to set pipeline complete for ${wt.phaseId}: ${err.message}`); }
      }
      log('ship', `Phase "${wt.phase.title}" — design review passed → complete`);

      // Reset design round counter on success
      try {
        const statusPath = resolve('.design/memory/status.json');
        let freshStatus = {};
        try { freshStatus = JSON.parse(readFileSync(statusPath, 'utf-8')); } catch (err) { logVerbose('ship', `Design status read failed: ${err.message}`); }
        freshStatus.round = 0;
        writeFileSync(statusPath, JSON.stringify(freshStatus, null, 2));
      } catch (err) { logVerbose('ship', `Design round reset failed: ${err.message}`); }
    }
  }
  ...
  log('ship', `Parallel design review complete: ${worktrees.length} phases processed`);
  return { success: anyProgress, madeProgress: anyProgress };
}
...
async function runDesignReview(phase) {
  ...
  const statusPath = resolve('.design/memory/status.json');
  let designStatus = { round: 0 };
  try { designStatus = JSON.parse(readFileSync(statusPath, 'utf-8')); } catch (err) { logVerbose('ship', `Design status read failed: ${err.message}`); }
  designStatus.round = (designStatus.round || 0) + 1;
  designStatus.phase = phase.title;
  designStatus.lastRun = new Date().toISOString();
  try {
  ...
  } catch (err) { logVerbose('ship', `Design status write failed: ${err.message}`); }
  ...
  if (!result?.success) {
    ...
    for (const [taskId, attemptId] of Object.entries(attemptIds)) {
      ...
      } catch (err) { logVerbose('ship', `Design attempt partial update failed for ${taskId}: ${err.message}`); }
    }
  ...
  for (const [taskId, attemptId] of Object.entries(attemptIds)) {
    ...
    } catch (err) { logVerbose('ship', `Design attempt outcome update failed for ${taskId}: ${err.message}`); }
  }
  ...
  if (hasShipBlockers) {
    ...
    try { freshStatus = JSON.parse(readFileSync(statusPath, 'utf-8')); } catch (err) { logVerbose('ship', `Design status re-read failed: ${err.message}`); }
    freshStatus.round = 0;
    try { writeFileSync(statusPath, JSON.stringify(freshStatus, null, 2)); } catch (err) { logVerbose('ship', `Design status reset write failed: ${err.message}`); }
  }

  // Re-read round from disk to return the design agent's value
  let finalStatus = { round: 0 };
  try { finalStatus = JSON.parse(readFileSync(statusPath, 'utf-8')); } catch (err) { logVerbose('ship', `Design final status read failed: ${err.message}`); }
...
function saveExecHistory(trigger, context) {
  ...
  } catch (err) { logVerbose('ship', `Exec history git log failed: ${err.message}`); entry.gitLog = '(unavailable)'; }
...
function loadExecEscalationCount() {
  ...
  } catch (err) { logVerbose('ship', `Escalation count read failed: ${err.message}`); return 0; }
}
...
function saveExecEscalationCount(count) {
  ...
  } catch (err) { logVerbose('ship', `Escalation count save failed: ${err.message}`); }
}
...
function ensureGitignoreEntries(entries) {
  ...
  } catch (err) { logVerbose('ship', `Gitignore update failed: ${err.message}`); }
}
...
function ensureGitRepo() {
  try {
    ...
    try {
      ...
      logVerbose('ship', `Initial commit skipped: ${err.message}`);
    }
...
const INFRA_ERROR_PATTERNS = [
  { pattern: /untracked working tree files would be overwritten/i, type: 'untracked-files' },
  { pattern: /your local changes.*would be overwritten/i, type: 'uncommitted-changes' },
  { pattern: /CONFLICT|merge conflict/i, type: 'merge-conflict' },
  { pattern: /not a git repository/i, type: 'git-init' },
  { pattern: /worktree.*already (exists|locked)/i, type: 'stale-worktree' },
  { pattern: /cannot lock ref|unable to create.*lock/i, type: 'git-lock' },
  { pattern: /fatal: pathspec.*did not match/i, type: 'missing-path' },
  { pattern: /ENOENT|no such file or directory/i, type: 'missing-file' },
  { pattern: /ENOSPC|no space left/i, type: 'disk-space' },
  { pattern: /branch.*already exists/i, type: 'stale-branch' },
  { pattern: /could not determine executable to run/i, type: 'missing-tool' },
  { pattern: /worktree.*is locked/i, type: 'worktree-lock' },
  // Catch-all for unrecognized git errors — ensures new error types still reach repair
  { pattern: /^fatal:/m, type: 'unknown-git-error' },
];
...
const repairAttempts = new Map(); // "phaseId:operation" → count

function readLogTail(n = 30) {
  try {
    const logPath = resolve('.ship/latest.log');
    if (!existsSync(logPath)) return '(no log file)';
    const content = readFileSync(logPath, 'utf-8');
    return content.split('\n').slice(-n).join('\n');
  } catch (err) { logVerbose('ship', `Log tail read failed: ${err.message}`); return '(could not read log)'; }
}

function appendRepairLog(repairId, errorType, errorMsg, operation, phaseTitle, status) {
  const repairsPath = resolve('.ship/repairs.md');
  const entry = [
    `## ${repairId} — ${errorType}`,
    `**When:** ${new Date().toISOString()}`,
    `**Operation:** ${operation} (phase: "${phaseTitle}")`,
    `**Error:** \`${errorMsg.slice(0, 300)}\``,
    `**Script:** .ship/repair-${repairId}.sh`,
    `**Diagnostics:** .ship/repair-diagnostics-${repairId}.log`,
    `**Status:** ${status}`,
    '',
  ].join('\n');

  try {
    const existing = existsSync(repairsPath) ? readFileSync(repairsPath, 'utf-8') : '# Infrastructure Repairs\n\n';
    writeFileSync(repairsPath, existing + entry + '\n');
  } catch (err) { logVerbose('ship', `Repair log write failed: ${err.message}`); }
}

async function attemptRepair(error, context) {
  const { operation, cwd: opCwd, phaseId, phaseTitle } = context;

  // Check repair budget
  const repairKey = `${phaseId || 'global'}:${operation}`;
  const attempts = (repairAttempts.get(repairKey) || 0) + 1;
  repairAttempts.set(repairKey, attempts);

  if (attempts > MAX_REPAIR_ATTEMPTS) {
    log('error', `Max repair attempts (${MAX_REPAIR_ATTEMPTS}) reached for ${operation} on "${phaseTitle}" — giving up`);
    return { repaired: false, reason: 'max-attempts' };
  }

  // Classify the error
  const errorType = classifyInfraError(error);
  if (!errorType) {
    log('ship', `Error is not infrastructure (not repairable): ${error.message?.slice(0, 200)}`);
    return { repaired: false, reason: 'not-infrastructure' };
  }

  log('ship', `Infrastructure failure: ${errorType} (attempt ${attempts}/${MAX_REPAIR_ATTEMPTS}) — spawning repair agent`);

  // Pause progress sync to prevent it from overwriting repair state fixes
  pauseProgressSync();

  // Build context for repair agent
  const logTail = readLogTail(30);
  const repairsPath = resolve('.ship/repairs.md');
  const priorRepairs = existsSync(repairsPath) ? readFileSync(repairsPath, 'utf-8') : '(none)';

  // Include diagnostics from prior repair attempts
  let priorDiagnostics = '';
  try {
    const diagFiles = readdirSync(resolve('.ship')).filter(f => f.startsWith('repair-diagnostics-'));
    for (const f of diagFiles.slice(-3)) { // last 3 diagnostics
      priorDiagnostics += `\n--- ${f} ---\n` + readFileSync(resolve('.ship', f), 'utf-8');
    }
  } catch (err) { logVerbose('ship', `Prior diagnostics read failed: ${err.message}`); }

  const repairId = `${Date.now()}`;
  const prompt = [
    `/repair ${errorType}`,
    '',
    `Repair attempt: ${attempts}/${MAX_REPAIR_ATTEMPTS}`,
    `Error: ${error.message}`,
    `Stderr: ${(error.stderr?.toString() || '').slice(0, 500)}`,
    `Operation: ${operation}`,
    `Working directory: ${opCwd || process.cwd()}`,
    `Phase: ${phaseTitle || '(none)'} (${phaseId || 'n/a'})`,
    `Repair ID: ${repairId}`,
    '',
    `Recent log:`,
    logTail,
    '',
    priorDiagnostics ? `Prior diagnostics:\n${priorDiagnostics}` : '',
    '',
    `Prior repairs:\n${priorRepairs}`,
  ].filter(Boolean).join('\n');

  const result = await runAgent(prompt, `Repair: ${errorType} (attempt ${attempts})`);

  // Execute the repair script
  const scriptPath = resolve(`.ship/repair-${repairId}.sh`);
  if (!existsSync(scriptPath)) {
    log('error', 'Repair agent did not produce a script at ' + scriptPath);
    appendRepairLog(repairId, errorType, error.message, operation, phaseTitle || '', 'NO-SCRIPT');
    return { repaired: false, reason: 'no-script' };
  }

  try {
    const scriptOutput = execSync(`bash "${scriptPath}"`, {
      cwd: opCwd || process.cwd(),
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'pipe'],
      timeout: REPAIR_SCRIPT_TIMEOUT_MS,
    });
    log('ship', `Repair script executed successfully`);
    logVerbose('ship', `Repair script output: ${scriptOutput.slice(0, 300)}`);
    appendRepairLog(repairId, errorType, error.message, operation, phaseTitle || '', 'SUCCESS');
    return { repaired: true, repairId };
  } catch (scriptErr) {
    log('error', `Repair script failed: ${scriptErr.message}`);
    appendRepairLog(repairId, errorType, error.message, operation, phaseTitle || '', 'SCRIPT-FAILED');
    return { repaired: false, reason: 'script-failed' };
  }
}
...
async function run(topic, options = {}) {
  ...
  writeStatus('running', { topic: topic || planName || 'resume' });
  if (phaseStopName) {
  ...
  let exitRepairAttempts = 0;

  // Outer loop: allows the premature-exit diagnostic to retry the pipeline
  // after fixing a logic bug. Inner while loop is the normal iteration loop.
  pipelineRetry: while (true) {
  ...
  while (!shouldStop) {
    ...
    // ── Human-required check (blocks until resolved) ──
    if (checkHumanRequired()) {
      log('ship', 'Pipeline paused — .ship/human-required.md exists. Remove it to resume.');
      shouldStop = true;
      break;
    }

    try { // Infrastructure error catch-all — routes to repair agent
    ...
    writeStatus('running', { iteration, phase: phase.title, phaseState: state });
    ...
    // ── Step 2d: Check for parallel design review ──

    if (!handledByParallel && state === 'awaiting-design') {
      const designPhases = findParallelDesignPhases(goals);
      if (designPhases && designPhases.length > 1) {
        result = await runParallelDesign(designPhases);
        handledByParallel = true;
      }
    }
    ...
    if (!handledByParallel) switch (state) {
      ...
      case 'awaiting-qa': {
        ...
        if (visualResult.regressions?.length > 0) {
          ...
          if (existsSync(PIPELINE_CLI)) {
            ...
            } catch (err) { log('error', `Failed to set pipeline qa-failed for ${phase.id}: ${err.message}`); }
          }
        }

        // Resolve forest warnings before moving on
        await resolveForestWarnings(phase.id);
          ...
          if (allTasksDone) {
            ...
            if (needsDesign && hasVisualFiles) {
              ...
              if (existsSync(PIPELINE_CLI)) {
                ...
                } catch (err) { log('error', `Failed to set pipeline awaiting-design for ${phase.id}: ${err.message}`); }
              }
              ...
              if (existsSync(PIPELINE_CLI)) {
                ...
                } catch (err) { log('error', `Failed to set pipeline complete for ${phase.id}: ${err.message}`); }
              }
        ...
        if (designResult.hasShipBlockers) {
          ...
          if (designRounds > MAX_DESIGN_ROUNDS) {
            ...
            if (existsSync(PIPELINE_CLI)) {
              ...
              } catch (err) { log('error', `Failed to set pipeline design-failed for ${phase.id}: ${err.message}`); }
            }
          }
        } else {
          // Design passed — mark phase complete
      ...
      case 'design-failed': {
        ...
        if (existsSync(PIPELINE_CLI)) {
          ...
          } catch (err) { log('error', `Failed to set pipeline awaiting-qa for ${phase.id}: ${err.message}`); }
        }
      ...
      case 'complete': {
        ...
        } catch (err) { logVerbose('ship', `Pre-rollup merge check failed: ${err.message}`); }
        ...
        if (Array.isArray(goals.majorPhases)) {
          ...
          if (completedMajor) {
            ...
            if (existsSync(fixesPath)) {
              ...
              try { unlinkSync(fixesPath); } catch (err) { logVerbose('ship', `Checkpoint fixes cleanup failed: ${err.message}`); }
            }
          }
        }

        // ── Phase 2 repair: ship permanent fixes if repairs were applied ──
        const repairsPath = resolve('.ship/repairs.md');
        if (existsSync(repairsPath)) {
          try {
            const repairContent = readFileSync(repairsPath, 'utf-8');
            if (repairContent.trim() && repairContent.includes('## ')) {
              log('ship', 'Infrastructure repairs were applied during this run — spawning repair agent for permanent fixes');
              const permanentResult = await runAgent(
                `/repair --permanent\n\nRepair history:\n${repairContent}\n\nImplement permanent fixes in lib/ files so these issues never recur. Read each repair entry, trace the root cause, implement the fix, run tests, and commit.`,
                'Permanent infrastructure fixes'
              );
              if (permanentResult?.success) {
                log('ship', 'Permanent infrastructure fixes applied');
                // Archive the repairs log
                try {
                  renameSync(repairsPath, resolve(`.ship/repairs-archive-${Date.now()}.md`));
                } catch (err) { logVerbose('ship', `Repairs archive rename failed: ${err.message}`); }
              }
            }
          } catch (err) {
            logVerbose('ship', `Phase 2 repair dispatch failed: ${err.message}`);
          }
        }
    ...
    if (!iterationHadProgress) {
      ...
      if (consecutiveNoProgress >= 3) {
        ...
        // CONTINUE means exec thinks progress is possible — dispatch resolver
        // for any QA-failed tasks on the current phase instead of stopping.
        if (phase) {
          const stuckTasks = getFailedTasks(phase);
          if (stuckTasks.length > 0) {
            log('ship', `Exec returned CONTINUE — dispatching resolver for ${stuckTasks.length} failed task(s)`);
            for (const ft of stuckTasks) {
              runDistiller('resolve', ft.id);
              await runAgent(`/resolve ${ft.title}`, `Resolving after zero-progress: ${ft.title}`);
            }
        ...
        log('error', 'Exec returned CONTINUE but no failed tasks to resolve — stopping');
        break;
      }
    } else {
    ...
    // Fix 8: Stagnation detection — progress is happening but the phase
    // isn't advancing. Logs a warning that the exit diagnostic can read.
    if (phase) {
      const currentState = getPipelineState(phase);
      if (currentState === lastPhaseState) {
        sameStateCount++;
        if (sameStateCount >= 5) {
          log('ship', `Stagnation warning: phase "${phase.title}" has been in state "${currentState}" for ${sameStateCount} consecutive iterations with progress — may be stuck in a loop`);
        }
      } else {
        sameStateCount = 0;
      ...
      // ── Infrastructure error catch-all ──
      // If the error is a git/filesystem infrastructure failure, route to
      // the repair agent instead of counting as no-progress.
      const infraType = classifyInfraError(iterationErr);
      if (infraType) {
        log('ship', `Infrastructure error caught in iteration ${iteration}: ${infraType}`);
        const repairResult = await attemptRepair(iterationErr, {
          operation: 'iteration',
          phaseId: phase?.id,
          phaseTitle: phase?.title,
        });
        if (repairResult.repaired) {
          log('ship', 'Repair succeeded — retrying iteration');
          iteration--; // retry same iteration number
          continue;
        }
        log('error', `Repair failed for ${infraType} — counting as no-progress`);
        consecutiveNoProgress++;
      } else {
        // Not infrastructure — rethrow for existing handling or log and continue
        log('error', `Unexpected error in iteration ${iteration}: ${iterationErr.message}`);
        consecutiveNoProgress++;
      }
    }
  }
  ...
  // If the pipeline stopped without completing all phases and this isn't a
  // dry-run, spawn the repair agent to diagnose the logic bug and fix it.
  // Up to 3 attempts — each gets the full log, goals state, and prior diagnostics.
  if (!pipelineSuccess && !dryRun && exitRepairAttempts < 3) {
    exitRepairAttempts++;
    log('ship', `Pipeline stopped prematurely (attempt ${exitRepairAttempts}/3) — spawning repair agent to diagnose`);
    writeStatus('repairing', { attempt: exitRepairAttempts, phase: phase?.title || '(none)' });

    // Pause progress sync to avoid state races during repair
    pauseProgressSync();

    // Snapshot state BEFORE repair to verify it actually changes something
    const preRepairGoalsHash = (() => {
      try { return readFileSync(GOALS_PATH, 'utf-8').length; } catch (err) { logVerbose('ship', `Pre-repair goals read failed: ${err.message}`); return 0; }
    })();
    const preRepairShipHash = (() => {
      try { return readFileSync(resolve('lib/ship.js'), 'utf-8').length; } catch (err) { logVerbose('ship', `Pre-repair ship.js read failed: ${err.message}`); return 0; }
    })();

    const repairId = `exit-${Date.now()}`;

    // Fix 1: Read FULL log for diagnose-exit (not just tail) — capped at 200KB
    let fullLog;
    try {
      const logPath = resolve('.ship/latest.log');
      const content = existsSync(logPath) ? readFileSync(logPath, 'utf-8') : '';
      fullLog = content.length > 200000 ? content.slice(-200000) : content;
    } catch (err) { logVerbose('ship', `Full log read failed: ${err.message}`); fullLog = '(could not read log)'; }

    const decisionsPath = resolve('.ship/decisions.log');
    const decisions = existsSync(decisionsPath) ? readFileSync(decisionsPath, 'utf-8').slice(-2000) : '(none)';
    const goalsState = readGoals();
    const stuckPhases = goalsState
      ? getAllPhases(goalsState).filter(p => p.status !== 'completed').map(p => {
          const failed = getFailedTasks(p);
          return `"${p.title}" (status: ${p.status}, pipeline: ${p.pipeline?.state || 'none'}, failed tasks: ${failed.length})`;
        }).join('\n  ')
      : '(no goals)';
    ...
    try {
      const diagFiles = readdirSync(resolve('.ship')).filter(f => f.startsWith('repair-diagnostics-'));
      for (const f of diagFiles.slice(-3)) {
        priorDiagnostics += `\n--- ${f} ---\n` + readFileSync(resolve('.ship', f), 'utf-8');
      }
    } catch (err) { logVerbose('ship', `Prior diagnostics read failed: ${err.message}`); }

    const prompt = [
      `/repair --diagnose-exit`,
      '',
      `Repair ID: ${repairId}`,
      `Exit repair attempt: ${exitRepairAttempts}/3`,
      `Iterations completed: ${iteration}`,
      `Active phase at exit: ${phase?.title || '(none)'} (${phase?.id || 'n/a'})`,
      '',
      `Stuck phases:`,
      `  ${stuckPhases}`,
      '',
    ...
    try {
      const diagResult = await runAgent(prompt, `Diagnose premature exit (attempt ${exitRepairAttempts})`);

      // Fix 3: Verify repair agent didn't break its own dispatch code
      try {
        execSync('node -c lib/ship.js', { encoding: 'utf-8', stdio: 'pipe' });
        // Check critical functions still exist
        const shipContent = readFileSync(resolve('lib/ship.js'), 'utf-8');
        const critical = ['attemptRepair', 'classifyInfraError', 'pipelineRetry'];
        const missing = critical.filter(fn => !shipContent.includes(fn));
        if (missing.length > 0) {
          log('error', `Repair agent removed critical functions: ${missing.join(', ')} — reverting lib/ship.js`);
          execSync('git checkout -- lib/ship.js', { stdio: 'pipe' });
        }
      } catch (syntaxErr) {
        log('error', `Repair agent broke lib/ship.js syntax — reverting: ${syntaxErr.message}`);
        execSync('git checkout -- lib/ship.js', { stdio: 'pipe' });
      }

      // Execute repair script if one was written
      const scriptPath = resolve(`.ship/repair-${repairId}.sh`);
      if (existsSync(scriptPath)) {
        try {
          execSync(`bash "${scriptPath}"`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'], timeout: REPAIR_SCRIPT_TIMEOUT_MS });
          log('ship', 'Exit repair script executed');
        } catch (scriptErr) {
          log('error', `Exit repair script failed: ${scriptErr.message}`);
        }
      }

      appendRepairLog(repairId, 'premature-exit', `Pipeline stopped at iteration ${iteration}`, 'diagnose-exit', phase?.title || '(none)', diagResult?.success ? 'SUCCESS' : 'PARTIAL');

      // Fix 2: Verify state actually changed before retrying
      const postRepairGoalsHash = (() => {
        try { return readFileSync(GOALS_PATH, 'utf-8').length; } catch (err) { logVerbose('ship', `Post-repair goals read failed: ${err.message}`); return 0; }
      })();
      const postRepairShipHash = (() => {
        try { return readFileSync(resolve('lib/ship.js'), 'utf-8').length; } catch (err) { logVerbose('ship', `Post-repair ship.js read failed: ${err.message}`); return 0; }
      })();
      const stateChanged = postRepairGoalsHash !== preRepairGoalsHash || postRepairShipHash !== preRepairShipHash;

      if (!stateChanged) {
        log('ship', 'Repair agent ran but nothing changed — skipping retry');
      } else if (diagResult?.success || diagResult?.madeProgress) {
        log('ship', 'Repair agent completed + state changed — retrying pipeline');
        shouldStop = false;
        pipelineSuccess = false;
        consecutiveNoProgress = 0;
        // Fix 5: Give the retry enough iteration runway
        iteration = Math.max(0, iteration - 10);
        continue pipelineRetry;
      }
    } catch (err) {
      log('error', `Exit diagnostic failed: ${err.message}`);
    }
  }

  // Fix 6: Ship permanent fixes even if pipeline didn't complete a major phase
  if (!pipelineSuccess) {
    const repairsPath = resolve('.ship/repairs.md');
    if (existsSync(repairsPath)) {
      try {
        const repairContent = readFileSync(repairsPath, 'utf-8');
        if (repairContent.trim() && repairContent.includes('## ')) {
          log('ship', 'Shipping permanent fixes from failed run repairs');
          const permanentResult = await runAgent(
            `/repair --permanent\n\nRepair history:\n${repairContent}\n\nImplement permanent fixes in lib/ files so these issues never recur. Read each repair entry, trace the root cause, implement the fix, run tests, and commit.`,
            'Permanent infrastructure fixes (end of failed run)'
          );
          if (permanentResult?.success) {
            log('ship', 'Permanent infrastructure fixes applied');
            try { renameSync(repairsPath, resolve(`.ship/repairs-archive-${Date.now()}.md`)); } catch (err) { logVerbose('ship', `Repairs archive rename failed: ${err.message}`); }
          }
        }
      } catch (err) { logVerbose('ship', `End-of-run permanent fixes failed: ${err.message}`); }
    }
  }

  break pipelineRetry; // No more retries — exit the outer loop
  } // end pipelineRetry

  // P2: Final review cycle when all phases complete
  if (pipelineSuccess) {
    log('ship', '── Final Review ──');
    writeStatus('post-completion', { step: 'final-review', iteration });
    ...
    // P3: Exec final review
    log('exec', 'Running executive final review');
    writeStatus('post-completion', { step: 'exec-final-review', iteration });
    runDistiller('exec', null);
    const finalReviewResult = await runAgent(
      '/exec --final-review',
      'Executive final review'
    );

    // Verify the exec wrote its outputs
    const finalReviewPath = resolve('.exec/memory/final-review.md');
    if (!existsSync(finalReviewPath)) {
      log('ship', 'Exec final review did not write .exec/memory/final-review.md — creating summary from available state');
      // Write a basic summary from what we have
      try {
        const goalsForSummary = readGoals();
        const phaseSummary = goalsForSummary
          ? getAllPhases(goalsForSummary).map(p => {
              const done = p.tasks?.filter(t => t.status === 'completed').length || 0;
              const total = p.tasks?.length || 0;
              return `- ${p.title}: ${done}/${total} tasks completed (${p.status})`;
            }).join('\n')
          : '(no goals data)';
        const vision = goalsForSummary?.vision || '(no vision set)';
        mkdirSync(resolve('.exec/memory'), { recursive: true });
        writeFileSync(finalReviewPath, [
          '# Project Final Review',
          '',
          `**Generated:** ${new Date().toISOString()}`,
          `**Note:** Auto-generated because exec final review did not produce a summary.`,
          '',
          '## Vision',
          vision,
          '',
          ...
          '',
          '## Recommendations',
          'Manual review recommended — exec final review did not complete successfully.',
          '',
        ].join('\n'));
        log('ship', 'Wrote fallback project summary to .exec/memory/final-review.md');
      } catch {}
    } else {
      log('ship', 'Exec final review completed — summary at .exec/memory/final-review.md');
    }

    // Park at awaiting-review — the human triggers the interactive session
    // when they're ready (via the sidebar app button or `/exec --final-review` in Claude Code)
    log('ship', 'Pipeline complete — awaiting human review. Run /exec --final-review in Claude Code');
    writeStatus('awaiting-review', {
      iteration,
      report: '.exec/memory/final-review.md',
      command: '/exec --final-review',
    });

    // Also write review state to .goals.json so the sidebar app can show the button
    try {
      const reviewGoals = readGoals();
      if (reviewGoals) {
        reviewGoals.review = {
        ...
        writeFileSync(GOALS_PATH, JSON.stringify(reviewGoals, null, 2));
      }
    } catch {}
  }
  ...
  // ── Token cost summary ──
  try {
    const { getCostSummary } = await import('./cost-tracker.js');
    const costs = getCostSummary();
    if (costs.dispatches > 0) {
      log('ship', `── Token Usage ──`);
      log('ship', `  ${costs.dispatches} dispatches | ${(costs.totalTokens / 1000).toFixed(0)}k tokens | ~$${costs.totalEstimatedCost.toFixed(2)} estimated`);
      for (const [agent, data] of Object.entries(costs.byAgent).sort((a, b) => b[1].estimatedCost - a[1].estimatedCost)) {
        log('ship', `    ${agent}: ${data.dispatches}x, ${((data.inputTokens + data.outputTokens) / 1000).toFixed(0)}k tokens, ~$${data.estimatedCost.toFixed(2)}`);
      }
    }
  } catch (err) { logVerbose('ship', `Cost summary failed: ${err.message}`); }
  ...
  const durationMin = Math.round((Date.now() - pipelineStartTime) / 60000);
  // Successful pipelines park at awaiting-review (set above). Only write
  // finished for failed runs or non-success exits.
  if (!pipelineSuccess) {
    writeStatus('finished', { success: false, iteration, duration: `${durationMin}m` });
  }
  ...
  while (i < raw.length) {
    ...
    switch (arg) {
      ...
      case '--restore': {
        ...
        if (isNaN(n) || n < 1) {
          console.error('--restore requires a positive iteration number, e.g. --restore 3');
          process.exit(1);
        }
        opts.restoreIter = n;
        break;
...
if (opts.restoreIter !== null) {
  restoreCheckpointCommand(opts.restoreIter);
  process.exit(0);
}

if (opts.review) {
  // Mark status as reviewing and tell the human how to start
  if (existsSync(STATUS_PATH)) {
    try {
      const status = JSON.parse(readFileSync(STATUS_PATH, 'utf-8'));
      if (status.state !== 'awaiting-review') {
        console.log(`Pipeline is in state "${status.state}" — review is only available when state is "awaiting-review".`);
        process.exit(1);
      }
    } catch {}
  }

  // Generate the briefing so the exec agent has full context
  runDistiller('exec', null);
  writeStatus('reviewing', { startedAt: new Date().toISOString() });

  // Update goals.json review state
  try {
    const reviewGoals = JSON.parse(readFileSync(GOALS_PATH, 'utf-8'));
    if (reviewGoals.review) reviewGoals.review.state = 'reviewing';
    writeFileSync(GOALS_PATH, JSON.stringify(reviewGoals, null, 2));
  } catch {}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  The exec wrote its assessment to:
    .exec/memory/final-review.md

  To start the interactive conversation, run this
  in your Claude Code session:

    /exec --final-review

  The exec will present findings, open the product
  in your browser, and ask: SHIP or IMPROVE?
...
Usage:
  ...
  node ship.js --dry-run <topic>   Show what would happen without running
  node ship.js --review             Start interactive final review (when pipeline is awaiting-review)
  node ship.js --checkpoints       List all checkpoint tags with dates
  node ship.js --restore <N>       Restore .goals.json and memory from iteration N
...
Examples:
  ...
  node lib/ship.js --checkpoints
  node lib/ship.js --restore 5
`);
  process.exit(0);
}
