 ...
 *
 * Hardening flags:
 *   --max-budget <dollars>   — stop when estimated cost exceeds limit (default: 20)
 *   --max-agents <n>         — stop after N agent dispatches (default: 15)
...
async function run(topic, options = {}) {
  const { resume, planName, dryRun, maxBudget, maxAgents, skipCanary, staleTimeoutMs } = options;

  const pipelineStartTime = Date.now();
  initLogFile();

  initCostTracker(maxBudget);

  log('ship', `Starting: ${topic || planName || 'resume from .goals.json'}`);
  log('ship', `Budget: $${maxBudget.toFixed(2)} | Max agents: ${maxAgents} | Stale timeout: ${Math.round(staleTimeoutMs / 60000)}min`);
  ...
  while (iteration < maxAgents) {
    iteration++;
    log('ship', `── Iteration ${iteration} ──`);

    // ── Budget check ──
    if (costTracker.isOverBudget()) {
      const cum = costTracker.getCumulative();
      log('error', `Budget exhausted ($${cum.total.toFixed(2)} / $${maxBudget.toFixed(2)})`);
      log('ship', `Pipeline paused at iteration ${iteration}. Run with higher budget or --resume to continue.`);
      break;
    }
    ...
    switch (state) {
      ...
      case 'qa-failed': {
        ...
        if (qaRounds > MAX_QA_ROUNDS) {
          if (replanCount >= MAX_PM_REPLANS) {
            ...
            iteration = maxAgents; // force exit
            break;
          }
          ...
          for (const taskTitle of failedTasks) {
            // Budget check before each fix dispatch
            if (costTracker.isOverBudget()) {
              const cum = costTracker.getCumulative();
              log('error', `Budget exhausted ($${cum.total.toFixed(2)} / $${maxBudget.toFixed(2)})`);
              break;
      ...
      case 'complete': {
        ...
        if (nextPhase) {
          ...
          pipelineSuccess = true;
          iteration = maxAgents; // clean exit
        }
        break;
      }
...
function parseArgs(argv) {
  ...
  while (i < raw.length) {
    ...
    switch (arg) {
      ...
      case '--max-budget':
        i++;
        opts.maxBudget = parseFloat(raw[i]);
        break;
      case '--max-agents':
        i++;
        opts.maxAgents = parseInt(raw[i], 10);
        break;
      case '--skip-canary':
...
Examples:
  node ship.js "extract formatCurrency to shared util"
  node ship.js --plan format-currency --max-budget 10
  node ship.js --resume --quiet
`);
  process.exit(0);
}
