  ...
  try {
    ...
    for (const task of phase.tasks) {
      ...
      if (taskFiles.length > 0) {
        ...
        // No files[] hint — check if task title keywords match changed file paths
        const keywords = task.title.toLowerCase().split(/\s+/).filter(w => w.length > 3);
        touched = keywords.some(kw =>
          changedFiles.some(cf => cf.toLowerCase().includes(kw))
        );
      }
...
async function runDesignReview(phase) {
  ...
  log('ship', 'Running design review...');
  const result = await runAgent(
  ...
  if (result?.success) {
    log('ship', 'Design review complete — findings in agent output');
  } else {
    log('ship', 'Design review did not complete — continuing anyway');
  }
  // Design review is advisory, not blocking
}
  ...
  while (!shouldStop) {
    ...
    if (state === 'needs-build') {
      ...
      if (parallelPhases && parallelPhases.length > 1) {
        const result = await runParallelBuilds(parallelPhases);
        if (result.madeProgress) consecutiveNoProgress = 0;
        else consecutiveNoProgress++;
        continue;
      }
    }
