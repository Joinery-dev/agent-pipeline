...
function commitWorktreeChanges(worktreePath, phaseTitle) {
  try {
    ...
    if (!status) {
      log('ship', `Worktree "${phaseTitle}": no uncommitted changes (builder likely committed directly)`);
      return false;
    }

    const fileCount = status.split('\n').length;
    log('ship', `Worktree "${phaseTitle}": committing ${fileCount} changed files`);
    execSync('git add -A', { cwd: worktreePath, stdio: 'pipe' });
    execSync(`git commit -m "parallel build: ${phaseTitle}"`, { cwd: worktreePath, stdio: 'pipe' });
    return true;
  } catch (err) {
    log('error', `Worktree commit failed for "${phaseTitle}": ${err.message}`);
    return false;
  }
}
