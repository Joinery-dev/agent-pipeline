...
async function runDesignReview(phase, isRevision = false) {
  // Only run if the project has UI pages
  const appDir = resolve('app');
  if (!existsSync(appDir)) return { hasShipBlockers: false };

  const label = isRevision ? 'Design re-review' : 'Design review';
  log('ship', `Running ${label}...`);
  ...
  if (!result?.success) {
    log('ship', `${label} did not complete — continuing`);
  ...
  if (hasShipBlockers && !isRevision) {
    log('ship', 'Design review found SHIP-BLOCKER issues — routing to PM for fix plan');
    logDecision(`Design review found SHIP-BLOCKER issues on "${phase.title}". Creating fix plan.`);

    // PM creates targeted fix plan
    const pmResult = await runAgent(
      `/pm:plan Design fixes for ${phase.title} — Read the design review output in .qa/screenshots/. Create targeted fix tasks for SHIP-BLOCKER issues only. Keep the existing plan, add a "Design Fixes" sub-section with specific tasks.`,
      `PM planning design fixes: ${phase.title}`
    );

    if (pmResult?.success) {
      // Builder implements fixes
      const buildResult = await runAgent(
        `/build ${phase.planFile || phase.title}`,
        `Building design fixes: ${phase.title}`
      );
      reconcileTaskStatuses(phase.id);

      // QA validates fixes
      const qaResult = await runAgent(
        `/qa ${phase.planFile ? phase.planFile.replace('plans/', '').replace('.md', '') : phase.title}`,
        `QA on design fixes: ${phase.title}`
      );
      reconcileQAStatuses(phase.id);

      // One more design review to confirm
      return await runDesignReview(phase, true);
    }
  } else if (hasShipBlockers && isRevision) {
    log('ship', 'Design re-review still has SHIP-BLOCKER issues — logging concern and continuing');
    logDecision(`Design revision for "${phase.title}" still has SHIP-BLOCKER issues. Logged as concern, continuing.`);

    // Save to PM concerns
    try {
      const concernsPath = resolve('.pm/memory/concerns.md');
      if (existsSync(concernsPath)) {
        const content = readFileSync(concernsPath, 'utf-8');
        const date = new Date().toISOString().split('T')[0];
        const entry = `\n## HIGH — Design issues on ${phase.title}\n**Opened:** ${date}\n**Status:** OPEN\n**Description:** Design review found SHIP-BLOCKER issues that persisted after one revision pass. Needs manual attention.\n`;
        writeFileSync(concernsPath, content + entry);
      }
    } catch {}
  } else {
...
function runQualityGate() {
  ...
  if (existsSync(concerns)) {
    ...
    if (openConcernMatches.length >= 3) {
      issues.push(`${openConcernMatches.length} OPEN concerns in PM memory — quality is accumulating debt`);
    }
  }
  ...
  if (existsSync(drift)) {
    ...
    if (driftingCount >= 2) {
      issues.push(`${driftingCount} active visual drift items — design language is fragmenting`);
    }
  }
