...
function runDistiller(agentType, taskId, { phaseId } = {}) {
...
function createBuildWorktree(phase) {
  const slug = phase.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+$/, '');
  const worktreePath = resolve(`.worktrees/build-${slug}`);
  const branchName = `parallel/${slug}-${Date.now()}`;
...
async function runParallelBuilds(phases) {
  ...
  for (const phase of phases) {
    try {
      ...
      const wt = createBuildWorktree(phase);
      wt.phaseId = phase.id;
      wt.phase = phase;
  ...
  while (!shouldStop) {
    ...
    if (!handledByParallel) switch (state) {
      ...
      case 'awaiting-qa': {
        ...
        // All checks passed (QA + visual + design) — now mark phase complete
        {
          ...
          if (allTasksDone) {
            ...
            log('ship', `Phase "${phase.title}" — QA, visual, and design checks all passed → complete`);
          }
        }
        break;
      }
