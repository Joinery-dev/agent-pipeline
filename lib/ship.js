...
const MAX_EXEC_RESTARTS = 3; // per pipeline run — after 3 exec restarts, truly stop
...
async function run(topic, options = {}) {
  ...
  while (!shouldStop) {
    ...
    if (iteration > 1 && !runQualityGate()) {
      ...
      if (execResult === 'RESTART') continue;
      shouldStop = true;
      break;
    }
      ...
      case 'qa-failed': {
        ...
        if (qaRounds > MAX_QA_ROUNDS) {
          if (replanCount >= MAX_PM_REPLANS) {
            ...
            if (execResult === 'RESTART') { replanCount = 0; break; }
    ...
    if (!iterationHadProgress) {
      ...
      if (consecutiveNoProgress >= 3) {
        ...
        if (execResult === 'RESTART') { consecutiveNoProgress = 0; continue; }
