...
/**
 * Read forest warnings from QA memory, dispatch resolver for each one.
 * Returns true if any warnings were found and resolved.
 */
async function resolveForestWarnings(phaseId) {
  ...
  log('ship', `${warnings.length} forest warning(s) found — routing to resolver`);
  ...
  await runAgent(
    `/resolve QA forest warnings need fixing for phase ${phaseId}. These are advisory findings that should be resolved before the phase is complete:\n\n${warningList}\n\nFix each warning. Do NOT change functionality — only address the specific issues listed.`,
    `Resolving ${warnings.length} forest warning(s)`
  );
