...
async function run(topic, options = {}) {
  ...
  if (pipelineSuccess) {
    ...
    // when they're ready (via the sidebar app button or `node lib/ship.js --review`)
    log('ship', 'Pipeline complete — awaiting human review. Run: node lib/ship.js --review');
    writeStatus('awaiting-review', {
      iteration,
      report: '.exec/memory/final-review.md',
      command: 'node lib/ship.js --review',
    });
  }
...
if (opts.review) {
  // Interactive final review — human triggers this when ready
  async function runInteractiveReview() {
    ...
    console.log('Starting interactive final review...');
    console.log('The exec agent will present findings and ask for your decision.\n');
    ...
    writeStatus('finished', { success: true, reviewCompleted: true });
    console.log('\nReview complete.');
  }
  runInteractiveReview().catch(err => {
