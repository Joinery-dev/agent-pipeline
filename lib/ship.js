...
function commitWorktreeChanges(worktreePath, phaseTitle) {
  try {
    const status = execSync('git status --porcelain', { cwd: worktreePath, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
...
function mergeWorktreeBranch(branchName, phaseTitle) {
  try {
    // Ensure transient workspace dirs are gitignored before merge.
    // Untracked files in .ship/ or .worktrees/ block merges from worktree branches.
    ensureGitignoreEntries(['.ship/', '.worktrees/']);

    // Stash uncommitted local changes before merging — the main working tree
    // may have modifications from progress sync, pipeline-cli updates, or
    // briefing generation that git refuses to overwrite during merge.
    let stashed = false;
    try {
      const status = execSync('git status --porcelain', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
      if (status) {
        execSync('git stash push -m "ship.js: pre-merge stash" --include-untracked', { stdio: 'pipe' });
        stashed = true;
        logVerbose('ship', 'Stashed local changes before merge');
      }
    } catch {}

    try {
      execSync(`git merge --no-ff "${branchName}" -m "merge parallel build: ${phaseTitle}"`, { stdio: 'pipe' });
      log('ship', `Merged parallel branch: ${branchName}`);
    } catch (err) {
      log('error', `Merge conflict for parallel branch ${branchName}: ${err.message}`);
      try { execSync('git merge --abort', { stdio: 'pipe' }); } catch {}
      if (stashed) { try { execSync('git stash pop', { stdio: 'pipe' }); } catch {} }
      return false;
    }
    ...
    if (stashed) {
      try {
        ...
        logVerbose('ship', 'Restored stashed changes after merge');
      } catch {
        // Stash pop conflict — the merge overwrote files that were also in the stash.
        // Drop the stash since the merged version is authoritative.
        log('ship', 'Stash pop conflict — using merged version (stash dropped)');
        try { execSync('git stash drop', { stdio: 'pipe' }); } catch {}
      }
    }

    return true;
  } catch (err) {
    log('error', `Merge failed for ${branchName}: ${err.message}`);
    return false;
  }
}
...
async function runParallelBuilds(phases) {
  ...
  // Phase 2: Launch builders in parallel (each in its own worktree)
  ...
  // Phase 3: Merge results back (sequential)
  ...
  for (const { wt, result } of results) {
    if (result?.success || result?.madeProgress) {
      ...
      const hasChanges = commitWorktreeChanges(wt.path, wt.phase.title);

      if (hasChanges) {
        const merged = mergeWorktreeBranch(wt.branch, wt.phase.title);
        if (merged) {
          anyProgress = true;
        ...
        // Builder may have committed directly — try merge anyway
        try {
          const ahead = execSync(`git rev-list HEAD..${wt.branch} --count`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
  ...
  // Reconcile .goals.json task statuses from worktrees
  if (mergedWorktrees.length > 0) {
    reconcileParallelGoals(mergedWorktrees);
  }

  // Run tests once after all merges
  if (anyProgress) {
    const testResult = runTestGate(1);
    if (!testResult.passed) {
      log('ship', 'Tests failed after parallel builds — resolver will handle');
    }
  }
