...
async function run(topic, options = {}) {
  ...
  if (!pipelineSuccess && !dryRun && exitRepairAttempts < 3) {
    ...
    try {
      ...
      // Reset loop state and retry via the outer pipelineRetry loop
      if (diagResult?.success || diagResult?.madeProgress) {
        log('ship', 'Repair agent completed — retrying pipeline');
        shouldStop = false;
