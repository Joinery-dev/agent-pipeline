...
async function run(topic, options = {}) {
  ...
  while (!shouldStop) {
    ...
    if (iteration > 1 && !runQualityGate()) {
      ...
      if (execResult === 'RESTART') { forceResearch = true; continue; }
      shouldStop = true;
      break;
    }
    ...
    if (!phase && Array.isArray(goals.majorPhases)) {
      ...
      if (emptyMajor) {
        ...
        if (!result.success) {
          ...
          if (execResult === 'RESTART') forceResearch = true;
      ...
      case 'qa-failed': {
        ...
        if (qaRounds > MAX_QA_ROUNDS) {
          if (replanCount >= MAX_PM_REPLANS) {
            ...
            if (execResult === 'RESTART') { replanCount = 0; forceResearch = true; break; }
    ...
    if (!iterationHadProgress) {
      ...
      if (consecutiveNoProgress >= 3) {
        ...
        if (execResult === 'RESTART') { consecutiveNoProgress = 0; forceResearch = true; continue; }
