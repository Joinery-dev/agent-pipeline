...
function restoreCheckpointCommand(n) {
  ...
  for (const f of files) {
    try {
      ...
      // File may not exist at that checkpoint
    }
  }
...
async function run(topic, options = {}) {
  ...
  if (!pipelineSuccess && !dryRun && exitRepairAttempts < 3) {
    ...
    const preRepairGoalsHash = (() => {
      try { return readFileSync(GOALS_PATH, 'utf-8').length; } catch { return 0; }
    })();
    const preRepairShipHash = (() => {
      try { return readFileSync(resolve('lib/ship.js'), 'utf-8').length; } catch { return 0; }
    })();
    ...
    } catch { fullLog = '(could not read log)'; }
    ...
    if (existsSync(repairsPath)) {
      try {
        ...
        if (repairContent.trim() && repairContent.includes('## ')) {
          ...
          if (permanentResult?.success) {
            log('ship', 'Permanent infrastructure fixes applied');
            try { renameSync(repairsPath, resolve(`.ship/repairs-archive-${Date.now()}.md`)); } catch {}
          }
        }
      } catch {}
    }
