...
function isPlaywrightAvailable() {
  try {
    ...
    console.error(`[playwright-gate] isPlaywrightAvailable: Playwright check failed: ${err.message}${err.stderr ? ' | stderr: ' + err.stderr.toString().slice(0, 200) : ''}`);
    return false;
  }
}
...
function runSpecs(testFiles) {
  ...
  try {
    ...
    try {
      ...
      console.error(`[playwright-gate] runSpecs: failed to parse Playwright JSON output: ${err.message}`);
    ...
    try {
      ...
      console.error(`[playwright-gate] runSpecs: failed to parse Playwright failure JSON: ${parseErr.message}`);
