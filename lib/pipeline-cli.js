...
try {
  switch (command) {
    ...
    case 'add-screenshot': {
      const phaseId = args[1];
      if (!phaseId) fail('Usage: add-screenshot <phaseId> --route "/" --imagePath <path> [--agent qa] [--viewport 1280x800]');
      const flags = parseFlags(args.slice(2));
      if (!flags.route) fail('--route is required');
      if (!flags.imagePath) fail('--imagePath is required');

      if (!existsSync(flags.imagePath)) fail(`Screenshot file not found: ${flags.imagePath}`);
      ...
      const screenshot = {
        ...
        route: flags.route,
        imagePath: flags.imagePath,
        agent: flags.agent || 'qa',
        createdAt: now,
      };

      if (flags.viewport) {
        const [w, h] = flags.viewport.split('x').map(Number);
        if (w && h) screenshot.viewport = { width: w, height: h };
      }

      if (!Array.isArray(phase.screenshots)) phase.screenshots = [];
      phase.screenshots.push(screenshot);

      writeGoals(goals);
      output({ screenshotId: screenshot.id, route: screenshot.route, phaseId });
      break;
    }

    default:
      fail(`Unknown command: ${command}\n\nCommands: validate, update-project, add-major-phase, update-major-phase, get-major-phase, add-phase, update-phase, add-task, update-task, get-task, get-phase, get-phase-full, find-task, add-attempt, update-attempt, update-status, set-pipeline, rollup, rollup-major, rollup-all, get-state, check-deps, stale-tasks, add-diagram, add-illustration, add-screenshot`);
  }
} catch (err) {
  fail(err.message);
}
