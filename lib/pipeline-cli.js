...
try {
  switch (command) {
    ...
    default:
      fail(`Unknown command: ${command}\n\nCommands: validate, find-task, get-task, get-phase, add-attempt, update-attempt, update-status, set-pipeline, rollup, rollup-major, rollup-all, get-state, check-deps, stale-tasks, add-phase, add-task, add-diagram, add-illustration`);
  }
} catch (err) {
  fail(err.message);
}
