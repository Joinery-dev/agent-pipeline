...
import {
  ...
  setPipelineState, getPipelineState, rollupPhaseStatus, checkDependencies,
  updateTaskStatus, getStaleTasks, getAllPhases, findEntityById,
} from './pipeline.js';
...
try {
  switch (command) {
    ...
    default:
      fail(`Unknown command: ${command}\n\nCommands: validate, find-task, get-task, get-phase, add-attempt, update-attempt, update-status, set-pipeline, rollup, get-state, stale-tasks, add-phase, add-task, add-diagram`);
  }
} catch (err) {
  fail(err.message);
}
