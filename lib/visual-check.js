  ...
  try {
    ...
    for (const r of results) {
      if (r.error && r.screenshot) {
        try { unlinkSync(resolve(r.screenshot)); deletedErrors++; } catch {}
      }
    }
    ...
    for (const dir of allDirs) {
      ...
      let files;
      try { files = readdirSync(dirPath).filter(f => f.endsWith('.png')); } catch { continue; }
    ...
    for (const [, entries] of hashMap) {
      ...
      for (const entry of entries.slice(0, -1)) {
        try { unlinkSync(entry.path); deletedDupes++; } catch {}
      }
    }
