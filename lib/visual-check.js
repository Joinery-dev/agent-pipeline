...
async function checkPages(pages, port) {
  ...
  // Clean up old screenshot dirs — keep only the latest per phase prefix
  try {
    const allDirs = readdirSync(SCREENSHOTS_DIR).filter(d => {
      try { return statSync(join(SCREENSHOTS_DIR, d)).isDirectory(); } catch { return false; }
    });
    // Group dirs by phase prefix (everything before the last timestamp segment)
    const groups = {};
    for (const dir of allDirs) {
      // Phase dirs: "{phaseId}-{timestamp}", non-phase: "{timestamp}"
      const lastDash = dir.lastIndexOf('-20');
      const prefix = lastDash > 0 ? dir.slice(0, lastDash) : '__no_phase__';
      if (!groups[prefix]) groups[prefix] = [];
      groups[prefix].push(dir);
    }
    // For each group, sort by name (timestamp is lexicographic), keep only latest
    for (const [, dirs] of Object.entries(groups)) {
      if (dirs.length <= 1) continue;
      dirs.sort();
      for (const old of dirs.slice(0, -1)) {
        try { rmSync(join(SCREENSHOTS_DIR, old), { recursive: true, force: true }); } catch {}
      }
    }
  } catch {}
...
async function main() {
  ...
  if (check.skipped) {
    ...
    if (server.started && server.proc) try { server.proc.kill(); } catch {}
    process.exit(2);
  }
  ...
  if (flags.mockupCompare) {
    ...
    if (illustrations.length > 0) {
      ...
      } catch { /* Playwright not available — comparisons still work, just no side-by-side PNGs */ }
