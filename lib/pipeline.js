...
const VALID_ATTEMPT_TYPES = ['build', 'qa', 'build-fix', 'qa-recheck'];
const VALID_PIPELINE_STATES = ['idle', 'building', 'awaiting-qa', 'qa-failed', 'complete'];
const VALID_AGENTS = ['pm', 'build', 'qa', 'resolve', 'design', 'exec'];
...
export function validateGoals(goals) {
  ...
  // Validate MajorPhases
  if (Array.isArray(goals.majorPhases)) {
    for (let mi = 0; mi < goals.majorPhases.length; mi++) {
      const mp = goals.majorPhases[mi];
      const mc = `MajorPhase[${mi}]`;

      if (typeof mp.id !== 'string') errors.push(`${mc}: id must be a string`);
      else checkId(mp.id, mc);
      if (typeof mp.title !== 'string') errors.push(`${mc}: title must be a string`);
      if (!VALID_STATUSES.includes(mp.status)) {
        errors.push(`${mc}: invalid status "${mp.status}"`);
      }
      if (!Array.isArray(mp.phases)) errors.push(`${mc}: phases must be an array`);
      if (typeof mp.order !== 'number') errors.push(`${mc}: order must be a number`);

      if (mp.diagrams) validateDiagrams(mp.diagrams, mc);
      if (mp.illustrations) validateIllustrations(mp.illustrations, mc);
    }
  }
  ...
  for (let pi = 0; pi < phases.length; pi++) {
    ...
    if (phase.pipeline && phase.pipeline.state !== undefined) {
      if (!VALID_PIPELINE_STATES.includes(phase.pipeline.state)) {
        errors.push(`${pc}: invalid pipeline.state "${phase.pipeline.state}"`);
      }
    }
...
export function setPipelineState(goals, phaseId, state, agentType) {
  ...
  if (agentType !== undefined && agentType !== null && !VALID_AGENTS.includes(agentType)) {
    throw new Error(`Invalid agent type: ${agentType}. Valid: ${VALID_AGENTS.join(', ')}`);
  }
