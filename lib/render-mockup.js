...
for (let i = 0; i < args.length; i++) {
  ...
  else if (args[i] === '--entityId' && args[i + 1]) flags.entityId = args[++i];
...
async function main() {
  ...
  try {
    ...
    console.error(`[render-mockup] Playwright import failed: ${err.message}`);
  ...
  // Auto-register in .goals.json if entityId and title provided
  let registered = false;
  if (flags.entityId && flags.title) {
    const cliPath = resolve('lib/pipeline-cli.js');
    if (existsSync(cliPath)) {
      try {
        const cliArgs = [
          cliPath, 'add-illustration', flags.entityId,
          '--title', flags.title,
          '--imagePath', flags.output,
          '--viewport', `${width}x${height}`,
        ];
        if (flags.html) cliArgs.push('--htmlSource', flags.html);
        execFileSync('node', cliArgs, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
        registered = true;
      } catch (err) {
        // Registration failed — still report the render success
        console.error(`[render-mockup] illustration registration failed for entity ${flags.entityId}: ${err.message}${err.stderr ? ' | stderr: ' + err.stderr.toString().slice(0, 200) : ''}`);
      }
    }
  }
