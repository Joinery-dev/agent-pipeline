...
const FIELD_REPORTS_DIR = resolve('.autoresearch/field-reports');
...
function ingestFieldReports() {
  if (!existsSync(FIELD_REPORTS_DIR)) return { reports: [], aggregate: null };

  const reports = [];
  for (const file of readdirSync(FIELD_REPORTS_DIR)) {
    if (!file.endsWith('.json')) continue;
    try {
      const report = JSON.parse(readFileSync(resolve(FIELD_REPORTS_DIR, file), 'utf-8'));
      if (report.reportVersion && report.summary) {
        reports.push(report);
      }
    } catch {}
  }

  if (reports.length === 0) return { reports: [], aggregate: null };

  // Aggregate failure categories across all projects
  ...
  for (const r of reports) {
    totalTasks += r.summary.totalTasks || 0;
    totalFirstPass += (r.summary.firstPassQARate || 0) * (r.summary.totalTasks || 0);

    for (const cat of r.failureAnalysis?.topCategories || []) {
      categoryCounts[cat.category] = (categoryCounts[cat.category] || 0) + cat.count;
      if (!categoryProjects[cat.category]) categoryProjects[cat.category] = new Set();
      categoryProjects[cat.category].add(r.projectHash);
    }
  }

  const universalFailures = Object.entries(categoryCounts)
    .map(([category, count]) => ({
      category,
      count,
      projects: categoryProjects[category]?.size || 0,
    }))
    .sort((a, b) => b.projects - a.projects || b.count - a.count);

  // Aggregate per-agent signal across projects
  const agentAgg = { builder: [], qa: [], resolver: [], pm: [] };
  for (const r of reports) {
    if (r.perAgent?.builder) agentAgg.builder.push(r.perAgent.builder);
    if (r.perAgent?.qa) agentAgg.qa.push(r.perAgent.qa);
    if (r.perAgent?.resolver) agentAgg.resolver.push(r.perAgent.resolver);
    if (r.perAgent?.pm) agentAgg.pm.push(r.perAgent.pm);
  }

  const perAgent = {};
  if (agentAgg.builder.length > 0) {
    const totalBuilderTasks = agentAgg.builder.reduce((s, b) => s + (b.tasks || 0), 0);
    const allBuilderFailures = {};
    for (const b of agentAgg.builder) {
      for (const f of b.topFailures || []) {
        allBuilderFailures[f.category] = (allBuilderFailures[f.category] || 0) + f.count;
      }
    }
    perAgent.builder = {
      targetBenchmarks: ['build-basic', 'build-api', 'build-component'],
      firstPassQARate: totalBuilderTasks > 0 ? +(agentAgg.builder.reduce((s, b) => s + b.firstPassQARate * (b.tasks || 0), 0) / totalBuilderTasks).toFixed(3) : null,
      avgBuildAttempts: +(agentAgg.builder.reduce((s, b) => s + parseFloat(b.avgBuildAttempts || 0), 0) / agentAgg.builder.length).toFixed(2),
      avgFixAttempts: +(agentAgg.builder.reduce((s, b) => s + parseFloat(b.avgFixAttempts || 0), 0) / agentAgg.builder.length).toFixed(2),
      topFailures: Object.entries(allBuilderFailures).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([category, count]) => ({ category, count })),
      tasks: totalBuilderTasks,
    };
  }
  if (agentAgg.qa.length > 0) {
    const totalQAAttempts = agentAgg.qa.reduce((s, q) => s + (q.totalAttempts || 0), 0);
    perAgent.qa = {
      targetBenchmarks: ['qa-accuracy', 'qa-false-positives'],
      totalAttempts: totalQAAttempts,
      avgSuccessRate: +(agentAgg.qa.filter(q => q.successRate !== null).reduce((s, q) => s + q.successRate, 0) / agentAgg.qa.filter(q => q.successRate !== null).length || 0).toFixed(3),
      tasks: agentAgg.qa.reduce((s, q) => s + (q.tasks || 0), 0),
    };
  }
  if (agentAgg.resolver.length > 0) {
    const totalResolveTasks = agentAgg.resolver.reduce((s, r) => s + (r.tasksRequiringResolve || 0), 0);
    perAgent.resolver = {
      targetBenchmarks: ['resolve-precision'],
      tasksRequiringResolve: totalResolveTasks,
      avgFixSuccessRate: +(agentAgg.resolver.filter(r => r.fixSuccessRate !== null).reduce((s, r) => s + r.fixSuccessRate, 0) / agentAgg.resolver.filter(r => r.fixSuccessRate !== null).length || 0).toFixed(3),
    };
  }
  if (agentAgg.pm.length > 0) {
    perAgent.pm = {
      targetBenchmarks: ['pm-planning', 'pm-review'],
      phasesCompleted: agentAgg.pm.reduce((s, p) => s + (p.phasesCompleted || 0), 0),
      avgQARoundsPerPhase: +(agentAgg.pm.filter(p => p.avgQARoundsPerPhase !== null).reduce((s, p) => s + parseFloat(p.avgQARoundsPerPhase || 0), 0) / agentAgg.pm.length).toFixed(2),
    };
  }
  ...
  return {
    ...
    aggregate: {
      ...
      totalTasks,
      avgFirstPassQARate: totalTasks > 0 ? totalFirstPass / totalTasks : null,
      universalFailures,
      perAgent,
...
function formatFieldReportSection(fieldData) {
  if (!fieldData.aggregate) return '';

  const { aggregate, reports } = fieldData;
  let section = `\n## Real-World Signal (from ${aggregate.projectCount} production pipeline${aggregate.projectCount > 1 ? 's' : ''})\n\n`;

  section += `**Aggregate:** ${aggregate.totalTasks} tasks across ${aggregate.projectCount} projects, ${((aggregate.avgFirstPassQARate || 0) * 100).toFixed(0)}% avg first-pass QA rate\n\n`;

  // Per-agent signal with benchmark routing
  if (aggregate.perAgent) {
    section += `### Per-Agent Signal (routes to specific benchmarks)\n\n`;

    const { builder, qa, resolver, pm } = aggregate.perAgent;

    if (builder) {
      section += `**Builder** → benchmarks: ${builder.targetBenchmarks.join(', ')}\n`;
      section += `- First-pass QA rate: ${(builder.firstPassQARate * 100).toFixed(0)}% across ${builder.tasks} tasks\n`;
      section += `- Avg build attempts: ${builder.avgBuildAttempts}, avg fix attempts: ${builder.avgFixAttempts}\n`;
      if (builder.topFailures.length > 0) {
        section += `- Top failures: ${builder.topFailures.map(f => `${f.category} (${f.count}x)`).join(', ')}\n`;
      }
      section += '\n';
    }

    if (qa) {
      section += `**QA** → benchmarks: ${qa.targetBenchmarks.join(', ')}\n`;
      section += `- QA accuracy: ${qa.successRate !== null ? (qa.successRate * 100).toFixed(0) + '%' : 'N/A'} success rate across ${qa.totalAttempts} attempts\n`;
      if (qa.recheckCount > 0) {
        section += `- Rechecks: ${qa.recheckCount} (${qa.recheckSuccessRate !== null ? (qa.recheckSuccessRate * 100).toFixed(0) + '% success' : 'N/A'})\n`;
      }
      section += '\n';
    }

    if (resolver) {
      section += `**Resolver** → benchmarks: ${resolver.targetBenchmarks.join(', ')}\n`;
      section += `- ${resolver.tasksRequiringResolve} tasks needed resolution, ${resolver.fixSuccessRate !== null ? (resolver.fixSuccessRate * 100).toFixed(0) + '% fixed' : 'N/A'}\n`;
      if (resolver.avgFixAttempts) section += `- Avg fix attempts: ${resolver.avgFixAttempts}\n`;
      section += '\n';
    }

    if (pm) {
      section += `**PM** → benchmarks: ${pm.targetBenchmarks.join(', ')}\n`;
      section += `- ${pm.phasesCompleted} phases completed`;
      if (pm.avgQARoundsPerPhase) section += `, avg ${pm.avgQARoundsPerPhase} QA rounds per phase`;
      if (pm.avgCompletionRate) section += `, ${(pm.avgCompletionRate * 100).toFixed(0)}% task completion rate`;
      section += '\n\n';
    }
  }

  if (aggregate.universalFailures.length > 0) {
    section += `### Top Failure Categories (all agents)\n`;
    for (const f of aggregate.universalFailures.slice(0, 5)) {
      section += `- **${f.category}**: ${f.count} occurrences across ${f.projects} project${f.projects > 1 ? 's' : ''}\n`;
    }
    section += '\n';
  }

  if (aggregate.recommendations.length > 0) {
    section += `### Field Recommendations\n`;
    for (const rec of aggregate.recommendations.slice(0, 5)) {
      section += `- "${rec}"\n`;
    }
    section += '\n';
  }
...
- Saturated: ${r.saturated ? 'yes' : 'no'}`).join('\n\n');
  ...
  // Read field reports from production pipelines
  const fieldData = ingestFieldReports();
  const fieldReportSection = formatFieldReportSection(fieldData);
...
1. **Write an evaluation report** analyzing:
   ...
   - **How tonight's results relate to real-world failure signal** (if field reports are available)
...
2. **Update priorities** — decide budget allocation for next night:
   ...
   - **Prioritize protocols whose weaknesses match real-world failure categories** from field reports
...
async function runNightlyCycle(options = {}) {
  ...
  nightlyState.usage = { inputTokens: totalInput, outputTokens: totalOutput, totalTokens: totalInput + totalOutput };
  saveState(nightlyState);

  log('done', '\n═══ Nightly Cycle Complete ═══');
  for (const r of results) {
    const status = r.error ? `ERROR: ${r.error}` : `score: ${r.bestScore?.toFixed(3) || '?'}${r.saturated ? ' (saturated)' : ''}`;
    const tokens = r.usage ? ` (${((r.usage.inputTokens + r.usage.outputTokens) / 1000).toFixed(0)}k tokens)` : '';
    log('done', `  ${r.target}: ${status}${tokens}`);
  }
  log('done', `  Total tokens: ${(totalInput / 1000).toFixed(0)}k input + ${(totalOutput / 1000).toFixed(0)}k output = ${((totalInput + totalOutput) / 1000).toFixed(0)}k total`);
