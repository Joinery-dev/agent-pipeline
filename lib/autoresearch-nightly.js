...
function formatFieldReportSection(fieldData) {
  ...
  if (aggregate.universalFailures.length > 0) {
    section += `### Top Failure Categories\n`;
  ...
  if (reports.length > 0) {
    section += `### Per-Project Summaries\n`;
    for (const r of reports) {
      const topFail = r.failureAnalysis?.topCategories?.[0];
      section += `- Project ${r.projectHash}: ${(r.summary.firstPassQARate * 100).toFixed(0)}% first-pass QA`;
      if (topFail) section += `, top failure: ${topFail.category}`;
      if (r.trends?.improving !== null) section += `, trend: ${r.trends.improving ? 'improving' : 'declining'}`;
      section += '\n';
    }
    section += '\n';
  }
