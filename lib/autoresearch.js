...
function readFieldReportSignal(targetFile) {
  ...
  // Determine which agent this benchmark targets from the protocol filename
  const agentMap = {
    'build': 'builder', 'build.md': 'builder',
    'qa': 'qa', 'qa.md': 'qa',
    'resolve': 'resolver', 'resolve.md': 'resolver',
    'pm': 'pm', 'pm:plan.md': 'pm', 'pm:review.md': 'pm',
    'exec': 'exec', 'exec.md': 'exec',
    'design-review': 'design', 'design-review.md': 'design',
  };
  const targetBase = (targetFile || '').replace(/^.*\//, '');
  const targetAgent = agentMap[targetBase] || agentMap[targetBase.replace('.md', '')] || null;

  let section = '';
  const totalTasks = reports.reduce((s, r) => s + (r.summary?.totalTasks || 0), 0);

  // Show per-agent signal matching this benchmark's target agent
  if (targetAgent) {
    const agentReports = reports.filter(r => r.perAgent?.[targetAgent]);
    if (agentReports.length > 0) {
      section += `\n## Real-World Signal for ${targetAgent} (from ${agentReports.length} projects, ${totalTasks} tasks)\n`;

      if (targetAgent === 'builder') {
        const allFailures = {};
        for (const r of agentReports) {
          for (const f of r.perAgent.builder.topFailures || []) {
            allFailures[f.category] = (allFailures[f.category] || 0) + f.count;
          }
        }
        const topFailures = Object.entries(allFailures).sort((a, b) => b[1] - a[1]).slice(0, 5);
        const avgRate = agentReports.reduce((s, r) => s + r.perAgent.builder.firstPassQARate, 0) / agentReports.length;

        section += `- First-pass QA rate: ${(avgRate * 100).toFixed(0)}% average\n`;
        if (topFailures.length > 0) {
          section += `- Top failure categories in production:\n`;
          for (const [cat, count] of topFailures) {
            section += `  - **${cat}**: ${count} occurrences\n`;
          }
        }
      } else if (targetAgent === 'qa') {
        const avgSuccess = agentReports.reduce((s, r) => s + (r.perAgent.qa?.successRate || 0), 0) / agentReports.length;
        section += `- QA success rate: ${(avgSuccess * 100).toFixed(0)}% average\n`;
      } else if (targetAgent === 'resolver') {
        const avgFix = agentReports.filter(r => r.perAgent.resolver?.fixSuccessRate !== null).reduce((s, r) => s + r.perAgent.resolver.fixSuccessRate, 0) / (agentReports.filter(r => r.perAgent.resolver?.fixSuccessRate !== null).length || 1);
        section += `- Fix success rate: ${(avgFix * 100).toFixed(0)}% average\n`;
      } else if (targetAgent === 'pm') {
        const avgRounds = agentReports.filter(r => r.perAgent.pm?.avgQARoundsPerPhase).reduce((s, r) => s + parseFloat(r.perAgent.pm.avgQARoundsPerPhase), 0) / (agentReports.filter(r => r.perAgent.pm?.avgQARoundsPerPhase).length || 1);
        section += `- Avg QA rounds per phase: ${avgRounds.toFixed(1)}\n`;
      }

      section += `\nYour hypothesis should address these real-world patterns.\nBenchmark scores matter, but real-world impact matters more.\n`;
    }
  }

  // Fall back to general failure signal if no agent-specific data
  if (!section) {
    const catCounts = {};
    ...
    const topFailures = Object.entries(catCounts).sort((a, b) => b[1] - a[1]).slice(0, 5)
      .map(([cat, count]) => `- ${cat}: ${count} occurrences`)
      .join('\n');

    if (topFailures) {
      section = `\n## Real-World Failure Signal (${reports.length} projects)\n${topFailures}\n`;
    }
  }
...
function buildProposerPrompt(program, currentProtocol, state, benchmarkTask, options = {}) {
  ...
  const fieldSignal = readFieldReportSignal(state.targetFile);
...
async function runProposer(program, currentProtocol, state, benchmarkTask, options = {}) {
  ...
  if (!result.success && !result.output) {
    return { success: false, hypothesis: 'Proposer agent crashed', newProtocol: null, usage: result.usage };
  }

  const parsed = parseProposerOutput(result.output);
...
async function runBenchmark(benchmark, targetFile, fullTargetPath, timeoutMs) {
  ...
  // Try .claude/ relative to CWD first (running from template/), then template/.claude/
  const templateClaude = existsSync(resolve('.claude'))
    ? resolve('.claude')
    : resolve('template', '.claude');
  ...
  // Read the current (possibly mutated) protocol and prepend it to the task prompt.
  // In production, ship.js invokes agents via slash commands (/build, /qa) which
  // Claude Code expands by reading .claude/commands/<agent>.md. Since autoresearch
  // uses -p <prompt> directly, we must inject the protocol content explicitly so
  // that protocol mutations actually affect agent behavior.
  let taskPrompt = benchmark.task;
  if (existsSync(targetInWorkspace)) {
    const protocolContent = readFileSync(targetInWorkspace, 'utf-8');
    taskPrompt = protocolContent + '\n\n---\n\n' + benchmark.task;
  }
...
async function run(targetFile, benchmarkName, options = {}) {
  ...
  while (state.iteration <= maxIterations && Date.now() < deadline && !shouldStop) {
    ...
    for (let c = 0; c < candidates; c++) {
      ...
      if (proposal.usage) {
        totalUsage.inputTokens += proposal.usage.inputTokens || 0;
        totalUsage.outputTokens += proposal.usage.outputTokens || 0;
      }
      ...
      if (benchResult.usage) {
        totalUsage.inputTokens += benchResult.usage.inputTokens || 0;
        totalUsage.outputTokens += benchResult.usage.outputTokens || 0;
      }
