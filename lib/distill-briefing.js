...
function buildQaBriefing() {
  ...
  // All majorPhase completion context (forest context)
...
function buildTaskBriefing() {
  ...
  // MajorPhase context (completion status)
  if (mp) {
    const mpCompletedCount = (mp.phases || []).filter(p => p.status === 'completed').length;
    const mpTotalCount = (mp.phases || []).length;
    parts.push(`<major-phase title="${esc(mp.title)}">${mpCompletedCount}/${mpTotalCount} phases done</major-phase>`);
  }
