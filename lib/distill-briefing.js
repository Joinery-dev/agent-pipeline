...
function safeRead(filePath) {
  try { return existsSync(filePath) ? readFileSync(filePath, 'utf-8') : null; }
    ...
    if (process.env.DEBUG) console.error(`[distill-briefing] safeRead failed for ${filePath}:`, err.message);
    return null;
  }
}
...
function safeJsonRead(filePath) {
  ...
  try { return JSON.parse(raw); } catch (err) {
    if (process.env.DEBUG) console.error(`[distill-briefing] safeJsonRead failed to parse ${filePath}:`, err.message);
    return null;
  }
}
...
function buildExecBriefing() {
  ...
  // QA state (loaded early — used by pattern scoping below)
