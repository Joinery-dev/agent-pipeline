...
function buildTaskBriefing() {
  ...
  // Patterns and regressions scoped to task files
  const fileScope = filesArg ? filesArg.split(',') : (targetTask?.files || []);
  const patterns = safeRead('.qa/memory/patterns.md');
  if (patterns && !patterns.includes('(none yet)')) {
    const scoped = filterEntriesByFiles(patterns, fileScope);
    if (scoped) {
  ...
  const regressions = safeRead('.qa/memory/regressions.md');
  if (regressions && !regressions.includes('(none yet)')) {
    const scoped = filterEntriesByFiles(regressions, fileScope);
    if (scoped) {
