...
async function getScreenshots() {
  ...
  try {
    ...
    console.error(`[screenshot-grid] failed to import visual-check.js: ${err.message}`);
  ...
  try {
    ...
    console.error(`[screenshot-grid] Playwright import failed: ${err.message}`);
  ...
  for (const p of [3000, 3001, 5173, 4321, 8080]) {
    try {
      ...
      // Port not listening — try next one
    }
  }
...
async function renderGrid(htmlPath, pngPath, screenshotCount) {
  ...
  try {
    ...
    console.error(`[screenshot-grid] renderGrid: Playwright import failed: ${err.message}`);
