...
function runRegressionRetirement() {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    for (let j = trajectory.length - 1; j >= 0; j--) {
      ...
      // A trajectory entry is "passing" if verdict is pass/PASS or checksPassing === checksTotal
      const passing = (t.verdict === 'pass' || t.verdict === 'PASS') ||
                      (t.checksPassing > 0 && t.checksPassing === t.checksTotal);
      if (passing) {
        consecutivePasses++;
      } else {
        break;
...
function runConcernsResolvedArchival() {
  ...
  for (const entry of entries) {
    ...
    const resolvedMatch = entry.match(/\*\*Resolved:\*\*\s*(\d{4}-\d{2}-\d{2})/);
